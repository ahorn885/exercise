...
# The "sport override" (framework_sport) gates which disciplines the bridge
...
def new_race():
    ...
    discipline_choices = _discipline_choices_for_race(
        db, initial_framework_sport, None
    )
...
def update_race(race_event_id: int):
    ...
    # #892 — re-map (don't wipe) the discipline narrowing when framework_sport
    # changes. Previously this cleared `included_discipline_ids` to None, which
    # read to the athlete as "the sport edit dropped all my disciplines"
    # (picker collapses to "Race-wide" / "not included in disciplines").
    # Canonical discipline IDs are global, so most prior picks stay valid for
    # the new sport — keep the still-valid subset and re-scope any terrain
    # couplings the same way. Only fires when framework_sport actually changed.
    ...
    if framework_sport_will_change:
        ...
        if dropped_disciplines:
            flash(
                'Sport override changed — disciplines that don’t apply to the '
                'new sport were dropped (' + ', '.join(dropped_disciplines) +
