...
| D-50 | **Phase 1 integration deployment** — schema migration + app code promotion (this repo, root) for `provider_auth`, `webhook_events`, `polar_*`, `wahoo_*`, `coros_*`, plus new columns on `cardio_log` and `training_log` per `Athlete_Data_Integration_Spec` §4–§6. **Schema shipped 2026-05-14** (v14): all 10 tables + 7 ALTER columns + 6 indexes appended to both `_SQLITE_MIGRATIONS` and `_PG_MIGRATIONS`. **PR1 (commit `3628ca6`):** `provider_auth.py` helper + COROS OAuth + `coros_ingest.py` + D-58 schema (`athlete_profile_field_provenance`, `account_nudges`, `disclosure_acknowledgments`). **PR2 (commit `686bb40`):** D-59/60/61 schema (`daily_availability_windows`, `gym_profiles`, `locale_equipment_overrides`, `locale_toggle_overrides`, 13 `locale_profiles` columns, `chain_registry.py`). **PR3 (merge `b819f0a`):** Polar OAuth + webhook + transaction-fetch ingestion (`routes/polar.py`, `routes/polar_ingest.py`) + three `cardio_log` partial-UNIQUE indexes (`polar_exercise_uidx`, `coros_label_uidx`, `wahoo_workout_uidx`) closing the PR1 §6 race window. **PR4 (this revision):** Account Config 1 Connections management tab on `/profile` (status / connect / disconnect / re-auth / `?<provider>_connected=1` passive-prefill prompt per v5 §A.2.7) + `provider_auth.disconnect` helper. | Med | 🟢 PR1 + PR2 + PR3 + PR4 shipped 2026-05-14 (commits `3628ca6`, `686bb40`, `b819f0a`, PR4-on-branch `claude/review-v5-pr3-handoff-PwMfn`); 🟡 frontend follow-on (D1 step-2 connect + D2 prefill UX + D3 locale-creation) pending | Production (Vercel `aidstation-pro.vercel.app` + TrueNAS Docker), AIDSTATION Wave-1 integration | Independent track. Lives in this repo (root `init_db.py`, `routes/`, `app.py`, `templates/profile/`). See `V5_Implementation_PR3_Closing_Handoff_v1.md` §5.1 for the PR5+ candidate menu carrying forward: **Option B partial** — Connections tab shipped PR4; full §A.2.5 re-onboarding prompt UX still owed; **Option D1** v5 frontend onboarding flow Step 2 connect + `?<provider>_connected=1` prompt (this PR ships the *passive notification* surface; D1 also needs the actual Step-2 connect form); **Option D2** `athlete_profile_field_provenance` prefill UX + `KNOWN_PROFILE_FIELDS` registry (Open Item #17); **Option D3** locale-creation flow with Mapbox chain detection from `chain_registry.py`; **Option E** 14-day connect-provider nudge background job; **Option F** Polar refresh-on-401 watch item; **Option G** `coros_ingest._ingest_activity` ON CONFLICT cleanup against the new `cardio_log_coros_label_uidx`. Recommended sequence: D1 → E → D2 → D3, with F + G as opportunistic drop-ins. Pre-deploy verification (PR1 §5.0 + PR3 §5.0): COROS env vars + redirect_uri; Polar env vars (`POLAR_CLIENT_ID`, `POLAR_CLIENT_SECRET`, `POLAR_WEBHOOK_SECRET`) + redirect_uri `https://aidstation-pro.vercel.app/polar/oauth/callback` + webhook URL registration via Polar partner API; spot-check `cardio_log_{polar_exercise,coros_label,wahoo_workout}_uidx` exist on Neon after first cold-start `_PG_MIGRATIONS` run. |
