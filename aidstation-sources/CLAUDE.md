...
| **4** | Plan generation + periodization validator (capped correction loop). | SPEC v1 DONE (§§1–13; §14 retro deferred to fresh-eyes session) |
...
- Backlog: `Project_Backlog_v31.md`
...
**Next forward move:** Andy's choice. Candidates with Layer 4 spec v1 landed:
- **Layer 4 §14 retrospective** — fresh-eyes critical-evaluation pass over §§1–13; lands before Layer 4 implementation per §12.6 deferral.
- **Layer 4 prompt-body design sessions** — five queued per stop-and-ask trigger #2: per-phase synthesizer (Pattern A), per-tier T1/T2 synthesizer (Pattern B refresh), single-session synthesizer (D-63), seam-reviewer (smallest scope; possible learning vehicle), race-week-brief. Andy picks order.
- **Layer 4 implementation track** — D-63 (on-demand workout) + D-64 (plan refresh tiers) implementation gates removed by spec landing; both ready for implementation against the §§1–13 contracts. Prompt bodies still need to land per the previous candidate; implementation can start with the deterministic-validator harness + payload schema scaffolding ahead of the prompts.
