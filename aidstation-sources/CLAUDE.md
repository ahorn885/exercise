...
| **4** | Plan generation + periodization validator (capped correction loop). | SPEC v1 DONE (§§1–13; §14 retro deferred to fresh-eyes session); PROMPT BODIES 2/5 (seam-reviewer + per-phase synth) |
...
- Backlog: `Project_Backlog_v32.md`
...
- Layer 4 prompt bodies (2 of 5 shipped; 3 queued — per-tier T1/T2, single-session, race-week-brief): `aidstation-sources/prompts/Layer4_SeamReviewer_v1.md`, `aidstation-sources/prompts/Layer4_PerPhase_v1.md`
...
**Next forward move:** Andy's choice. Candidates with Layer 4 spec v1 + 2/5 prompt bodies landed (per-prompt cadence now in effect; each remaining prompt is its own session under stop-and-ask trigger #2):
...
- **Layer 4 prompt-body design sessions (3 remaining)** — per-tier T1/T2 synthesizer (D-64; Pattern B refresh; may split T1+T2 across 2 sessions), single-session synthesizer (D-63; Pattern B; smallest scope of the remaining 3), race-week-brief (Pattern B; pre-race only). Andy picks order. Conventions inherited from `Layer4_SeamReviewer_v1.md` + `Layer4_PerPhase_v1.md`: tool-use output mechanism, extended thinking, hybrid input format, closed-set coaching-flag schema-enum enforcement, 14-section file structure, schema-enforced length caps, coaching voice.
- **Layer 4 implementation track** — D-63 (on-demand workout) + D-64 (plan refresh tiers) implementation gates removed by spec landing; both ready for implementation against the §§1–13 contracts. Prompt bodies for D-63 + D-64 (single-session + per-tier T1/T2) still need to land before LLM-call layer can be implemented; deterministic-validator harness + payload schema scaffolding can start independently.
