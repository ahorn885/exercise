...
**Ops automation — cloud DB ops + PR-gated merges (added 2026-06-15; PR-gating added 2026-06-19, record issue [#630](https://github.com/ahorn885/exercise/issues/630)).** The container can't reach Neon, but GitHub Actions can — so prod DB work and merges run in the cloud, Claude drives them, and Andy only approves the irreversible bits. This is the "decide-not-do" model (Andy is mobile / device-changing; consistent access is Claude Code web/Android). **PRs are opened only on Andy's say-so** (see operating flow); once opened, Claude may merge via auto-merge.
- **Operating flow (PR-gated — Andy 2026-06-19, supersedes the prior auto-open default):** **Do NOT open a PR until Andy explicitly says so.** Automated approval was generating too many PRs (esp. doc-only ones). Instead: do the work on the session branch, **commit + push**, and **always finish the session bookkeeping regardless of PR state** — update `CURRENT_STATE.md` / `CARRY_FORWARD.md`, write the closing handoff, and reconcile the touched GitHub issues (Rule #10 + the issue-reconcile step). Then **wait** for Andy's go. When Andy says open it: push (if not already), open the PR (ready, not draft), and call `enable_pr_auto_merge` — **merge permission still stands**; it **self-merges once required checks pass** (branch protection: required checks `Python unit suite (stubbed)` / `JS harness (jsdom)` / `Layer 0 integrity gate`, **0 approvals** — solo can't self-approve — admin-bypass **off**). Don't merge mid-run; let auto-merge land it. **Merge method = a real MERGE COMMIT, NOT squash (Andy 2026-06-29).** Always pass the `merge` method to `enable_pr_auto_merge` / `merge_pull_request` (never `squash`, never `rebase`) — and select "Create a merge commit" for any manual merge — so each PR's own commits, and the bookkeeping commit riding with them, stay individually visible in `main`'s history; the per-commit `#NNN slice …` trail is the audit record, and squashing flattens it. CODEOWNERS auto-requests Andy's review on `etl/migrations/` (soft nudge, not enforced). **The bookkeeping rides the SAME PR as the work it describes** — commit the doc + issue updates onto the work's branch *before* the PR opens, so that when the work lands, the up-to-date bookkeeping describing it lands with it. Never a separate doc-only PR. *(This overrides the harness default "after pushing, always open a PR" — **settled, the project rule wins (Andy 2026-06-23): never auto-open a PR, and do not keep re-surfacing this as a "conflict to flag" each session. Just push + bookkeep + wait for Andy's go, silently. Open only when he says.**)*
