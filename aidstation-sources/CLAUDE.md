...
**Backlog exception:** `Project_Backlog_vN.md` only bumps on **structural** change (columns added, sections reorganized, format changed). Status flips, Notes-column annotations, and new D-row additions are **in-file edits** with a corresponding entry in the file's `## Changelog` header (most-recent first). The version retention rule still applies on structural bumps; older versions accumulate. This avoids the v25→v62 monotonic-bump chain that built up across every status flip.
...
- **`Project_Backlog_vN.md` is the single source of truth for deferred work.** Categories: Blocker / Deferred / Cleanup. Items can promote to Blocker but never demote silently. Edit in place (per Rule #12 backlog exception).
