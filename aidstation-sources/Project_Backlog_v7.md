...
**Started:** 2026-05-10
**File revision:** v7 — 2026-05-13 (post-FC-3 closure: Spec v5 schema corrections + §5 query-layer rewrite; D-40 / D-42 / D-44 closed; D-41 carried with retry note)
...
| D-41 | **`terrain_types` 7 enrichment columns enumeration.** Drift report §2.10 names them (`terrain_id`, `category`, `requires_elevation`, `technical_surface`, `environment`, `simulatable`, `simulation_note`) + secondary UNIQUE on `(terrain_id, etl_version)` but v3/v4 vocabulary §4.14 still references the 2-col v2 spec. 16 superseded rows out of 31 indicate hand-curation provenance worth investigating during the same schema-dump session. | Low | 🟢 Cleanup | 2B (terrain mapping) | **FC-3 retry pending.** `\d layer0.terrain_types` failed in FC-3 on a client-side quoting artifact (the `\d` shorthand got fragmented). Retry uses `information_schema.columns` form: `SELECT column_name, data_type, is_nullable, column_default FROM information_schema.columns WHERE table_schema = 'layer0' AND table_name = 'terrain_types' ORDER BY ordinal_position;` + a parallel `pg_constraint` query for UNIQUE. Once output lands, str_replace `Layer0_ETL_Spec_v5` §4.14 sub-block. |
...
| D-46 | **`sport_discipline_bridge` row-multiplication audit.** Follow-up from D-44. If any active `exercise_db_sport` in `layer0.sport_name_aliases` maps to multiple `framework_sport` values at the same `etl_version`, the §4.11 bridge derivation will produce multiplied rows (one per alias mapping). Need to determine whether this is intentional curation (e.g., a sport like "Trail Running" legitimately belongs to multiple framework sports) or accidental dict duplication in `sport_name_aliases.py`. | Med | 🟢 Cleanup | §4.11 bridge derivation, 2A consumption | Run: `SELECT exercise_db_sport, COUNT(*) FROM layer0.sport_name_aliases WHERE superseded_at IS NULL GROUP BY exercise_db_sport HAVING COUNT(*) > 1;`. If results: review each multi-mapped row; if accidental, tighten `sport_name_aliases.py` and re-run ETL. If intentional, document why in §4.16 + audit `sport_discipline_bridge` row counts to confirm no downstream double-counting. |
...
### Session FC-4 (next): tentative scope
- **D-41 retry** — `terrain_types` schema dump using `information_schema.columns` form; str_replace `Layer0_ETL_Spec_v5` §4.14 sub-block. ~10 min.
- **D-46 audit** — `sport_name_aliases` multi-mapping COUNT query; if results, decide tighten-dict vs document-intentional. ~10 min.
- **D-03 / D-07** — ETL parser fixes pending v20 re-run (CC task; can ship in parallel with FC-4 spec-side work).
- **Layer 3 design kickoff** — primary forward move. Layer 3 is six parallel sub-prompts (race analysis, fitness capacity, training history, injury/risk analysis with HITL trigger generation, goal alignment, constraint mapping) plus the Layer 3.5 hard HITL gate. First session likely focused on the discovery doc / sub-prompt boundary decisions before per-sub-prompt spec work begins.
