...
| D-50 | **Phase 1 integration deployment** — schema migration + app code promotion (this repo, root) for `provider_auth`, `webhook_events`, `polar_*`, `wahoo_*`, `coros_*`, plus new columns on `cardio_log` and `training_log` per `Athlete_Data_Integration_Spec` §4–§6. **Schema shipped 2026-05-14** (v14): all 10 tables + 7 ALTER columns + 6 indexes appended to both `_SQLITE_MIGRATIONS` and `_PG_MIGRATIONS`; local SQLite idempotent re-run verified. `DATABASE.md` updated. **PR1 route wiring shipped 2026-05-14** (commit `3628ca6`): `provider_auth.py` provider-agnostic helper + COROS as first real OAuth (start/callback/webhook) + `coros_ingest.py` payload dispatcher + D-58 schema additions (`athlete_profile_field_provenance`, `account_nudges`, `disclosure_acknowledgments`). | Med | 🟢 PR1 shipped 2026-05-14 (commit `3628ca6`); 🟡 follow-on PRs pending | Production (Vercel `aidstation-pro.vercel.app` + TrueNAS Docker), AIDSTATION Wave-1 integration | Independent track. Lives in this repo (root `init_db.py`, `routes/`, `app.py`). See `V5_Implementation_PR1_Closing_Handoff_v1.md` §5.1 for the PR2+ candidate menu: **Option A** D-59/60/61 schema PR (shipped 2026-05-14 per PR2 handoff — `daily_availability_windows`, `gym_profiles`, `locale_equipment_overrides`, `locale_toggle_overrides`, 13 `locale_profiles` columns, `chain_registry.py`); **Option B** COROS frontend + Account Config 1 management screen; **Option C** next provider OAuth (Polar recommended; tokens NULL-expiry + per-user webhook HMAC); **Option D** v5 frontend onboarding flow (defer); **Option E** 14-day connect-provider nudge job. Recommended sequence: A → C → B → D. Pre-deploy verification (PR1 §5.0): confirm COROS Open API surface + register `redirect_uri` `https://aidstation-pro.vercel.app/coros/oauth/callback` + set `COROS_CLIENT_ID` / `COROS_CLIENT_SECRET` Vercel env vars. |
