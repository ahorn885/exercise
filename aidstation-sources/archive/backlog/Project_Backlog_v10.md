...
| D-50 | **Phase 1 integration deployment** — schema migration + Vercel app code promotion for `provider_auth`, `webhook_events`, `polar_*`, `wahoo_*`, `coros_*`, plus new columns on `cardio_log` and `training_log` per `Athlete_Data_Integration_Spec` §4–§6. Scoped + ready in CC chat; deployment timing: anytime after `Athlete_Data_Integration_Spec` v1 lands (now). | Med | 🟡 Deferred | Production, Vercel app, AIDSTATION Wave-1 integration | Independent track. Owner: CC. Coordinate with `DATABASE.md` updates. Per L3-Spec-Trio decision: deploy whenever the spec looks right; non-blocking and additive. |
...
| D-55 | **Garmin migration onto `provider_auth`.** Confirmed 2026-05-13: drop legacy `garmin_auth` table; Garmin gets a row in the unified `provider_auth` shape. `garth` session JSON migrates to new `session_blob TEXT` column on `provider_auth`. Breaking change for existing Garmin-connected users (one-time re-auth). | Med | 🟡 Deferred | Vercel app `routes/garmin.py`, `garmin_connect.py` | Migration order per `Athlete_Data_Integration_Spec` §2.3: (1) deploy `provider_auth` with `session_blob`; (2) one-shot data migration `garmin_auth → provider_auth`; (3) route code switches reads/writes; (4) drop `garmin_auth`. Coordinate with D-50 (Phase 1 integration deployment). |
...
### Session next: Layer 3 spec writing
