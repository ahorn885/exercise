...
- **CORRECTION (the "one active toggle" claim above was WRONG): there are 12 active gear toggles today** — `Classic XC ski setup`, `Skate XC ski setup`, `Climbing — roped`, `Bouldering`, `Rappelling / abseiling`, `Via ferrata`, `Mountaineering`, `Whitewater paddling setup`, `Fencing setup`, `Shooting setup`, `Snowshoeing setup` (+1). The catalog **names are already scoped** (Andy: "we already scoped this"); **§10's proposed grid is WITHDRAWN — it reinvented existing toggles** (ski gear already = the two XC setups). Two further facts from the live data: (a) only **3** have their discipline-unlock wired (`Climbing — roped`→D-012, `Rappelling`→D-013, `Snowshoeing`→D-017); the other 9 gate nothing; (b) **all 12 have `paired_equipment_categories = {}`** — the equipment-pool mechanism is deliberately unused (#623 retired individual "assumed" equipment items; the `Mountaineering kit` item was retired *into* the toggle). So **gear toggles unlock disciplines/workout-types, not an equipment pool** — Andy's intent, confirmed by the data.
- **MODEL CORRECTION (supersedes §4.4 Option A, §5.2, §10 — to be rewritten in v2):** since gear toggles carry **no** equipment (only discipline-unlocks), the only craft-specific attribute is **terrain**. So the unified model collapses to **one table** (Andy's Option B is correct): athlete-owned gear → **disciplines unlocked** (merging `craft_discipline_aliases` + `gated_discipline_ids`), with **terrain compatibility** as a craft-only extra column; `paired_equipment_categories` is **dropped**. The real cascade gap: today **only crafts + skill toggles gate feasibility**; gear toggles only emit a 2C `toggle_off_for_discipline` *coaching flag* — to make "gear unlocks workout-types" real, gear must feed the **same feasibility cascade** as crafts. "Refine, don't invent" — reuse `craft` + `gear toggle`; coin nothing.
- **Still open (Andy):** (1) confirm single-table + routing gear through the feasibility cascade; (2) populate the mostly-empty gear→discipline wiring now (Trigger #2) vs per-discipline later; (3) is any craft-vs-gear-toggle discriminator even surfaced, or is it just "has terrain or not"?
