# Catalog Migration Plan — v1

**Version:** 1.0
**Status:** Draft. Strategy-level v1 — sequencing and key decisions only. Per-phase execution detail to be added as each phase approaches.
**Last updated:** 2026-05-13 (L3-Spec-Trio session)
**Owner:** D-52 (Project_Backlog)
**Cross-references:**
- `Control_Spec_v6.md` §2 (architectural framing)
- `Athlete_Data_Integration_Spec.md` §2.4 (target state)
...
**Deliverable:** Field-by-field mapping document for each catalog: `public.exercise_inventory ↔ layer0.exercises`, `public.equipment_items ↔ layer0.equipment_items`.
...
3. Audit `public.exercise_inventory.exercise` values against `layer0.exercises.name` values. Identify perfect matches, near-matches, and unmatched. Output: alias table seed.
4. Repeat for equipment: `public.equipment_items.tag` ↔ `layer0.equipment_items.canonical_name`.
5. Decide naming convention for per-user denormalized fields (continue using app-style names, or adopt canonical names).

**Estimated effort:** 1-2 sessions. Independent of any app or pipeline work.
...
**Estimated effort:** Largest phase. Each table is its own migration with backfill, dual-write window, cutover, drop. Likely 1-2 sessions per table.
