# Layer 0 ETL Specification — v5

**Version:** 5.0 (file revision); schema version v3 (unchanged — Layer 0 schema didn't fork; v5 folds FC-3 Neon-verified schema corrections and the §5 query-layer narrative refresh into the v4 consolidation)
**Status:** Revised 2026-05-13 (FC-3). v5 closes D-40 (`terrain_gap_rules` schema enumerated), D-42 (`cross_sport_properties` `confidence` type corrected to TEXT), and D-44 (`sport_name_aliases` UNIQUE corrected to 3-col, one-to-many inverse claim retracted). §5 query layer rewritten to mirror the per-layer 2A–2E spec signatures verbatim; §5.3 canonical payload updated for `movement_components` + `common_injury_patterns` renames. D-41 (`terrain_types` schema enumeration) deferred — Neon query failed on a client-side quoting artifact; carried forward in `Project_Backlog_v7`.
**Supersedes:** `Layer0_ETL_Spec_v4.md` (FC-2, 2026-05-12) and the v3 patch chain folded into it.
**Sources:**
...
4. **§4.14 `terrain_types` 7-column enumeration** still pending — FC-3 Neon query failed on a client-side `\d` quoting artifact. Retry uses `information_schema.columns` form (see `Project_Backlog_v7` D-41).
...
- Project Backlog (`Project_Backlog_v7`) is the active cross-layer tracker; spec catches up to backlog decisions as they land.
...
**Consumer impact (D-44):** §4.11 `sport_discipline_bridge` derivation should treat the alias join as one-to-many. If an active `exercise_db_sport` maps to multiple `framework_sport` rows, the bridge build will produce one row per (exercise_db_sport, framework_sport, discipline) triple — possibly multiplying expected row counts. Investigate at next bridge re-derivation; if the multi-mapping is unintentional curation (a row should map to exactly one framework sport in practice), tighten the source `.py` dict, not the UNIQUE constraint.
...
**Active tracker:** `Project_Backlog_v7.md` is the single source of truth for cross-layer drift items, blockers, deferred work, and cleanup. This section preserves the v3 enumeration for historical continuity but does not maintain a parallel live list. Refer to the backlog for current state.
...
**v4 reconciliation summary — items now in `Project_Backlog_v7.md`:**
