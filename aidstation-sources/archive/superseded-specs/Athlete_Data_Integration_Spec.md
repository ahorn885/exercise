# Athlete Data Integration Spec — v1

**Version:** 1.0
**Status:** Draft. First version. Cross-cutting spec — not a Layer N spec; follows its own structure rather than the 14-section node-spec depth standard (Control_Spec §8.3, post-v6 amendment).
**Last updated:** 2026-05-13 (Initial draft, post-L3-Discovery handoff)
**Source decisions:**
...
**Cross-references:**
- `DATABASE.md` (app schema source of truth for `public.*`)
- `Layer0_ETL_Spec_v7.md` (layer0 schema source of truth)
- `PROVIDERS_SCHEMA.md` (producer-side integration design — lives in the Vercel-app codebase)
- `HANDOFF-2026-05-13.md` and `HANDOFF-2026-05-13-stub-batch.md` (provider stub deployment playbooks — Vercel-app codebase)
- `L3-Discovery_Pre-Spec-Trio_Inventory` (schema dump informing this spec — produced this session)

---

## 1. Purpose

Defines the consumer-side data model for AIDSTATION pipeline access to athlete integration data. Where `PROVIDERS_SCHEMA.md` is the *producer* spec (what the Vercel-app integration layer writes to Neon), this is the *consumer* spec (what Layer 1 sourcing, Layer 3 nodes, and downstream layers see when they query that data).
...
### 2.3 Garmin migrates onto `provider_auth`; legacy `garmin_auth` dropped

`PROVIDERS_SCHEMA.md` originally exempted `garmin_auth` from the new generic pattern on the basis that the `garth` library uses username/password (not OAuth) and didn't fit. **This spec reverses that decision.** The legacy `garmin_auth` table is dropped; Garmin gets a row in `provider_auth` like every other provider.

The `garth` session JSON blob — currently in `garmin_auth.garth_session` — is stored on the unified `provider_auth` row. Two options for where:
...
This is a breaking migration for any existing Garmin-connected users — they re-authenticate once during the migration window. Acceptable cost. Migration order:

1. Deploy `provider_auth` (with `session_blob` column) and `webhook_events` (per §5).
2. Write a one-shot data migration: for each `garmin_auth` row, insert a corresponding `provider_auth` row with `provider='garmin'`, copy `garth_session` → `session_blob`, NULL out OAuth fields.
3. Update `routes/garmin.py` and `garmin_connect.py` to read/write `provider_auth` instead of `garmin_auth`.
4. Drop `garmin_auth` and remove from `init_db.py`'s migration list.

Backlog row D-55 captures the work.
...
### 2.5 PG-only — SQLite backend deprecated

Confirmed 2026-05-13. The dual-backend pattern in `init_db.py` (parallel `_PG_MIGRATIONS` and `_SQLITE_MIGRATIONS` lists, dual-type strategy in DATABASE.md §3) collapses to PG-only as part of the catalog migration (Option A). The `layer0.*` schema uses PG-specific types (`TEXT[]` arrays, server-side defaults via `NOW()`) that SQLite cannot represent without lossy conversion.

**Implication for this spec:** Integration table CREATE statements use PG types directly. No SQLite variants are specified. The `_SQLITE_MIGRATIONS` list in `init_db.py` is frozen — no new entries — and eventually removed as part of the Option A migration.
...
|---|---|---|---|---|
| Garmin | Live ingestion (FIT files); migrating to `provider_auth` | `garth` session (user/pass) | FIT files: cardio activities + wellness samples | D-55 migration to `provider_auth` |
...
**Wave-1 priority for AIDSTATION pipeline consumers:** Garmin (already live), Polar, COROS, Wahoo. These four cover the strength/wellness/recovery axis (Polar + COROS for HRV-driven recovery, Garmin for training load, Wahoo for plan push) and unlock the highest-confidence 3A `recent_trajectory` outputs.

**Wave-2:** Strava, Whoop, TrainingPeaks, Zwift, RWGPS.

**Out of scope for v1 spec:** Apple Health, Samsung Health (require platform-specific clients beyond the Vercel-app architecture).
...
### 5.4 Garmin (post-migration)

No new per-provider table for Garmin — existing `garmin_workouts` and `wellness_log` continue serving. Only change is `garmin_auth` → `provider_auth` migration per §2.3.
