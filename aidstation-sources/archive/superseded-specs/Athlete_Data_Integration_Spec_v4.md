# Athlete Data Integration Spec — v4

**Version:** 4.0
**Status:** Draft v4. Cross-cutting spec — not a Layer N spec; follows its own structure rather than the 14-section node-spec depth standard (Control_Spec §8.3, post-v6 amendment).
**Last updated:** 2026-05-14 (v4: §2.5 carve-out — D-50 Phase 1 SQLite block ratified as a one-time documented exception to the `_SQLITE_MIGRATIONS` freeze; freeze remains in force for all future tables)
**Supersedes:** `Athlete_Data_Integration_Spec_v3.md` (2026-05-14)
**Source decisions:**
...
- `DATABASE.md` (app schema source of truth for `public.*`; repo root)
...
**Documented exception (v4, 2026-05-14):** The D-50 Phase 1 ship (commit `909dc17`) added 147 lines + 7 ALTERs to `_SQLITE_MIGRATIONS` covering `provider_auth`, `webhook_events`, the four `polar_*`, `wahoo_plans`, the three `coros_*`, and the seven `cardio_log`/`training_log` foreign-id columns. This was an override of the freeze and should have been a stop-and-ask under CLAUDE.md triggers #5 and #8. Surfaced in `D50_Phase1_Schema_Review_v1.md`. Andy ratified retroactively under Option A:

- The override stays in place (the lines are inert when `DATABASE_URL` is set, which is the dev path).
- The freeze rule remains in force for all subsequent integration tables and any other schema additions. No further `_SQLITE_MIGRATIONS` entries.
- The D-50 SQLite block ages out with the rest of the SQLite path at the D-54 PG-only collapse — no separate cleanup needed.

This carve-out is one-time. Any future schema work that wants to violate the freeze again must go through Andy first.
