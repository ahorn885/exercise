...
**Spec docs:**
- `Layer0_ETL_Spec_v3.md` (the canonical doc; needs v4 rewrite per drift report)
- `Layer0_Deployed_Schema_and_Drift_Report.md` (current state of truth)
- `Layer0_ETL_Spec_v3_Patch_Batch_B.md`, `Layer0_ETL_Spec_v3_Patch_Batch_C.md` (incremental patches)

**Source data:**
- `Sports_Framework_v10.xlsx` (0A)
- `AR_Exercise_Database_v19.xlsx` (0B)
- `Vocabulary_Audit_v2.md` (0C)
...
Every Layer 2 node queries Layer 0 via the **query layer** — 11 typed Python functions (`q_layerXN_*_payload`), one per consumer node. The LLM never writes SQL. Query layer pattern documented in `Layer0_ETL_Spec_v3.md` §5.
...
`Project_Backlog.md` is the single rolling tracker for cross-layer drift, deferred items, and cleanup tasks. Updated at every layer/node boundary. Categories: Blocker / Deferred / Cleanup. See `Project_Backlog.md` for current state.
...
Every layer and sublayer (Layer 0, Layer 1, Layer 2A-E, Layer 3, Layer 4, Layer 5) gets its own consolidated spec doc named `LayerNX_Spec.md`. Depth standard set by `Layer2C_Spec.md`:
...
Where things currently live, what's pending.

### Layer 0
- ✅ `Layer0_ETL_Spec_v3.md` — canonical (needs v4 rewrite per drift report)
- ✅ `Layer0_Deployed_Schema_and_Drift_Report.md` — current truth
- ✅ Batch A, B, C patches — incremental
- ⏳ `Layer0_ETL_Spec_v4.md` — pending FC-2

### Layer 1
- ✅ `Athlete_Onboarding_Data_Spec_v2.md` — partial (§§A-C/G-H/M/N + §§I/J/K/L complete; §§D-F pending)
- ✅ `Section_I_Audit.md` — §I structured form audit (drafted; 10 v3 polish candidates parked as D-25)
...
- ✅ `Adherence_Drop_Spec_v2.md` — Plan Management subsystem (predecessor work; Plan Management spec proper is D-27)
- ⏳ Remaining onboarding sections (§§D-F) — pending

### Layer 2
- ✅ `Layer2A_Spec.md` — backfilled 2026-05-10
- ✅ `Layer2B_Spec.md` — backfilled 2026-05-10
- ✅ `Layer2C_Spec.md` — drafted 2026-05-10
- ✅ `Layer2D_Spec.md` — drafted 2026-05-10
- ✅ `Layer2E_Spec.md` — drafted 2026-05-11

### Layer 3+
- ⏳ `Layer3_Spec.md` — not yet started (HITL gate design)
- ⏳ `Layer4_Spec.md` — not yet started (plan generation)
- ⏳ `Layer5_Spec.md` — not yet started (supplemental outputs)

### Cross-cutting
- ✅ `Control_Spec.md` — this doc
- ✅ `Project_Backlog.md` — rolling tracker

### Architecture predecessors (historical context)
- `Training_App_Architecture_Handoff.md` — original architecture notes
- `Layer0_to_PlanGen_Contract_Preview.md` — early contract sketch
- `Query_Layer_Spec_Handoff.md` — query layer design
- Various `*_Handoff.md` files — session bookkeeping
...
- **Handoff docs are not specs.** They're session bookkeeping. Once a node is "locked," migrate the decisions to its `LayerNX_Spec.md`.
