...
| 2B-3 | `§H.2 Race Terrain Type` field must use TRN-xxx IDs as controlled vocabulary | Layer 1 onboarding spec | Open Item E from 2B locking; 🟡 **Partial-close 2026-05-20** (Phase 5.1 form-refresh A) — race-event edit path (`/profile/race-events/<id>/edit`) now captures terrain breakdown as TRN-xxx + percentage rows persisted to `race_events.race_terrain JSONB`; orchestrator threads into Layer 2B. Onboarding §H.2 step-3c terrain capture surface still owed (form-refresh B follow-on); Layer 2B `_validate_inputs` still rejects empty race_terrain loudly (separate loosen-for-empty follow-on). |
