...
| H | Away craft availability — (b) craft↔location + (c) craft attached to a travel event | **DESIGNED** (2026-06-14, `designs/Event_Windows_Design_v1.md`) — reframed twice: away-craft is one axis of a missing **event-window** surface (Andy: not just travel — also home-constraint windows like indoor-only-for-weather/childcare; v2 has none, v1 `plan_travel.indoor_only` was left behind). Forks F1/F2/F4 ratified; F3 revised **per-day/sub-week** (not per-week); F8 category equipment baselines (hotel assumed-then-logged, Trigger #2) + F9 home-constraint windows added; race locations split to their own issue. **Re-sliced into 5** (Slice 1 home-constraint vertical → away locale → baselines → away craft → UX). DDL owed Andy's hands | follow-up PR |
...
**WS-H — away craft availability → DESIGNED 2026-06-14 (`designs/Event_Windows_Design_v1.md`); Slice 1 BUILT 2026-06-14 (`designs/Event_Windows_Slice1_HomeWindows_Spec_v1.md`, branch `claude/compassionate-keller-ej5tcx`).** The design surfaced that away-craft can't exist without a missing date-windowed environment surface; **Andy then reframed it from "away/travel" to "event windows"** — date-bounded periods where the training environment differs, covering BOTH away locations AND **home-constraint** windows (the athlete is home but can't train normally — weather, or kids needing supervision → indoor-only / no-outdoor-cardio). v2 has none; v1 had exactly this (`plan_travel.indoor_only` via `/coaching/review`, on the now-retired `home/hotel/partner/airport` enum) and it was **left behind**.
- **Ratified forks (Andy):** F1 athlete-level + **plan-gen review/edit/append panel** with **inline new-location create**; F2 destination = a **reused `locale_profiles` row** (*"travel is how we build our crowd-sourced locations database"*, differentiator #8); F4 away craft **none unless declared**.
- **F3 REVISED — per-DAY / sub-week** (was per-week): Andy may be in **3-4 locations in one week** (work travel), so a window is date-ranged (often <1 week) and each session maps to its date's environment. The **grid(weekly counts)↔synthesis(per-day placement)** seam is the core new machinery / main risk.
- **F8 NEW — category equipment baselines (Trigger #2):** a not-yet-seen hotel gym gets an **assumed** baseline; athlete logs actuals on arrival → **T1/T2 regen**. Scoped to **commercial / hotel / climbing gyms** — the *current* `routes/locales.py:205 MANUAL_CATEGORIES` (NOT the retired `home/hotel/partner/airport` enum — Andy corrected my framing). Baseline contents need sign-off.
- **F9 NEW — subtractive home windows = Slice 1:** `indoor_only` (home, no outdoor — weather/childcare) **+ `locale_unavailable`** (home cluster minus one closed locale — gym remodel / park flooded; Andy). The `indoor_only`/`no_outdoor_cardio` split was **collapsed to one flag** (Andy: same thing); in-transit `reduced_volume` **deferred to #593**.
- **F-race → SEPARATE ISSUE (Andy):** race locations are real (Boston stable; AR year-to-year) but **treated differently** from training locations — LLM-infer the area's terrain+weather to suggest "train for <terrain>" + feed plan-gen. Out of scope here; filed separately.
- **Re-sliced into 5** (home-constraint → away locale + inline create → category baselines/Trigger #2 → away craft/the literal WS-H → UX). Each ≤5 files, independently shippable. **Slice 1 BUILT** (`designs/Event_Windows_Slice1_HomeWindows_Spec_v1.md`): `athlete_event_windows` (`indoor_only` + `locale_unavailable`) + the existing cascade resolved **per date-segment** (`orchestrator._build_event_window_overlay`) + `compute_event_windows_hash` (both plan keys, no-windows byte-identical) + the date-scoped synthesis overlay (Trigger-#1 wording signed off) + minimal capture UI. Create-path only (refresh-overlay render deferred, mirroring #540→#557); counts stay on home (spec §4 limitation). Suite green 2418/30. **Seam settled (Andy 2026-06-14):** the existing cascade resolved **per date-segment** — a window is just different terrain/equipment inputs ("no different from normal proxies/substitutions"); the earlier overflow arithmetic was removed. **DDL owed Andy's hands** (Neon egress blocked).
