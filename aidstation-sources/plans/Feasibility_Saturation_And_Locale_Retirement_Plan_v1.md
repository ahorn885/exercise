...
- **Re-sliced into 5** (home-constraint → away locale + inline create → category baselines/Trigger #2 → away craft/the literal WS-H → UX). Each ≤5 files, independently shippable. **Owed before Slice-1 build:** the F3 seam contract + the `home_constraint` flag set. DDL owed Andy's hands (Neon egress blocked).
