...
- **F8 NEW — category equipment baselines (Trigger #2):** a not-yet-seen hotel gym gets an **assumed** baseline (resurrects hotel/airport/partner as equipment templates, not location identity); the athlete logs actuals on arrival → **T1/T2 regen** with real equipment + brought craft. Baseline contents need sign-off.
- **F9 NEW — home-constraint windows:** the smallest end-to-end vertical (no location/craft) → **Slice 1**.
