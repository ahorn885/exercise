# Catalog Migration Plan — v3

**Version:** 3.0
**Status:** Draft. Strategy-level v3 — SQLite-retirement decoupling. Per-phase execution detail to be added as each phase approaches.
**Last updated:** 2026-05-16 (v3: SQLite backend retirement decoupled from this plan — shipped independently in PR13. §1 out-of-scope clarification + §5 decision #5 marked Resolved. Plan scope unchanged otherwise.)
**Supersedes:** `Catalog_Migration_Plan_v2.md` (v2, 2026-05-13), `Catalog_Migration_Plan.md` (v1, 2026-05-13)
**Owner:** D-52 (Project_Backlog)
**Cross-references:**
- `Control_Spec_v7.md` §2 (architectural framing)
- `Athlete_Data_Integration_Spec_v5.md` §2.4 (target state)
...
**SQLite retirement decoupled from this plan.** v2 §1 out-of-scope listed SQLite backend support as "explicitly deprecated as part of this migration (D-54)"; v2 §5 decision #5 said "Drop entirely as part of this migration." Both framed D-54 as something that would land alongside the `public.*` → `layer0.*` migration. That coupling was never load-bearing — D-54 was just temporally scheduled to fold into Phase 5 to amortise the migration window. PR13 (2026-05-16) retired the SQLite path independently: stripped `SQLITE_SCHEMA`, `_SQLITE_MIGRATIONS`, `init_sqlite`, `sqlite_path`, all `_is_postgres()` runtime guards; PG-only via `DATABASE_URL`. D-54 ✅ Resolved.

**Plan scope unchanged.** Phase 5 remains "drop `public.*` catalogs." Phases 1–4 unchanged. The migration is now purely about `public.*` → `layer0.*` catalog reads; the SQLite path is no longer part of the scenery.
...
- SQLite backend support — retired independently in PR13 (2026-05-16); see D-54 ✅ Resolved. No longer coupled to this plan.
...
5. **SQLite backend:** ✅ Resolved — retired independently in PR13 (2026-05-16). `_SQLITE_MIGRATIONS`, `SQLITE_SCHEMA`, `init_sqlite`, `sqlite_path`, all `_is_postgres()` runtime guards stripped; `database.py` PG-only. No longer coupled to this migration.
