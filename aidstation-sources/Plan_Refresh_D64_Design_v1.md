...
| 4 | Default cascade per tier | **T1: Layer 3A + Layer 3B + Layer 4 (next 2 days)** (3B re-run added 2026-05-17 per `Layer4_Spec.md` §4.3-wins amendment — Pattern B's validator reads phase intent for the intensity-distribution check; the prior "T1 doesn't re-run 3B" framing was retired). **T2: Layer 3A + Layer 3B + Layer 4 (next 7 days).** **T3: Layer 2A/B/C/D/E + Layer 3A/B/C/D + Layer 4 (next 28 days).** NL intent can ADD upstream re-runs to any tier (e.g., T1 + "I just hurt my ankle" triggers 2D + 3A + 3B + Layer 4). | Tier scope drives cascade depth. T1's 3B re-run was added 2026-05-17 because Layer 4's deterministic validator reads phase intent for every refresh tier (the `intensity_dist_*` rule + the dominant-phase intended_volume_band lookup). The prior framing — "2-day horizon doesn't justify 3B since periodization operates on weekly granularity" — confused the *decision* surface (weekly) with the *contract* surface (validator). 3B is cheap; landing it on every tier matches the validator. NL intent is additive — it never reduces the cascade, only expands. |
...
**Default cascade:** Layer 3A re-eval + Layer 3B re-eval + Layer 4 (with `scope_days=2, start_date=today_or_next_uncompleted`).

**Amended 2026-05-17 (§4.3-wins resolution):** prior cascade was 3A-only with periodization shape inherited from adjacent-session `phase_metadata`; the paired amendment to `Layer4_Spec.md` §3.2 (signature) made `layer3b_payload` required on every tier because Pattern B's validator reads phase intent for the intensity-distribution check + the dominant-phase intended_volume_band lookup. The prior-narrative "T1 doesn't justify 3B since periodization operates on weekly granularity" conflated the decision-surface (weekly) with the contract-surface (validator). 3B is now re-run on T1 to honor the validator contract; downstream consumers (T1 prompt body `Layer4_RefreshT1_v2.md` §3.5 + §3.8) updated accordingly.
