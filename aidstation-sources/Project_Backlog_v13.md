...
**Started:** 2026-05-10
**File revision:** v13 — 2026-05-14 (Layer3_3B_Spec.md shipped same session as the reconciliation pass; new Rule #13 added to CLAUDE.md — every closing handoff names CLAUDE.md as the first re-read)
...
| D-50 | **Phase 1 integration deployment** — schema migration + app code promotion (this repo, root) for `provider_auth`, `webhook_events`, `polar_*`, `wahoo_*`, `coros_*`, plus new columns on `cardio_log` and `training_log` per `Athlete_Data_Integration_Spec` §4–§6. None of these tables exist yet in `init_db.py`. Scoped + ready; deployment timing: anytime after `Athlete_Data_Integration_Spec_v3` (current) is read. | Med | 🟡 Deferred | Production (Vercel `aidstation-pro.vercel.app` + TrueNAS Docker), AIDSTATION Wave-1 integration | Independent track. Lives in this repo (root `init_db.py`, `routes/`, `app.py`). Coordinate with `DATABASE.md` updates and `PROVIDERS_SCHEMA.md` (which already documents the target shape for `provider_auth` and `webhook_events`). Per L3-Spec-Trio decision: deploy whenever the spec looks right; non-blocking and additive. v1 has no users (per Andy 2026-05-14) — strangler-fig sequencing is safe. |
