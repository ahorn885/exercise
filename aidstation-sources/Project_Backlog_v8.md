...
**Started:** 2026-05-10
**File revision:** v8 — 2026-05-13 (post-FC-4a closure: Spec v6 closes D-41 terrain_types schema + D-46 sport_name_aliases multi-mapping audit; new cleanup D-47 for bridge-multiplication rationale comment)
...
### Session FC-4b (next): tentative scope
- **D-21 retry** — `health_condition_categories` column-name dump via `information_schema.columns`. ~5 min. Suggested query:
  ```sql
  ...
  ```
  Once output lands, str_replace `Layer0_ETL_Spec_v6` §4.14 D-21 bullet with the deployed column-name reconciliation, and update Layer 2D §5.2 + Layer 2E references if any column-name fix is required.
- **D-03 / D-07** — ETL parser fixes pending v20 re-run (CC task; orthogonal to spec-side work).
- **D-47 rationale comment fix** — fold into next Layer 2D revision (`Layer2D_Spec` → v2 if material; or piggyback on Layer 3 design when 3D depends on 2D output).
- **Layer 3 design kickoff** — primary forward move. Layer 3 is six parallel sub-prompts (race analysis, fitness capacity, training history, injury/risk analysis with HITL trigger generation, goal alignment, constraint mapping) plus the Layer 3.5 hard HITL gate. First session likely focused on the discovery doc / sub-prompt boundary decisions before per-sub-prompt spec work begins. **Recommended for a fresh chat** — biggest scope item; deserves a dedicated context window.
