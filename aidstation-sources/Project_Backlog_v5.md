...
| D-39 | **`Vocabulary_Audit` v3 rewrite** — formal canonical body parts list update. Two cleanup items folded together: (1) `Collarbone` added to Section 1 Shoulder region per D-23 curation (2026-05-12); (2) "Total: 41 canonical body parts" header is stale — enumerated tables sum to 51 with Collarbone (50 before). Both surfaced during D-23 work. | Low | 🟢 Cleanup | Layer 1 v3, future vocab work | Doesn't block any current work — D-23 migration validates against the full 51-token canonical set; deployed `body_parts_at_risk` column already includes Collarbone. Rewrite consolidates the documentation. Folds well with Layer 1 §B onboarding refinements (D-25). |
