...
**Status:** File-revision v8 — 2026-05-16 (post-PR13 SQLite + TrueNAS retirement; retroactive cleanup of v6's "removed during Phase 5" SQLite-coupling narrative; §9 doc map staleness explicitly flagged with Rule #12 resolve-to-highest-N reminder; no body changes to §§1–8 or §§10–11). v7 closed 2026-05-14 (post-L3-Spec-Trio Round 2; §9 doc map sync after Onboarding v4 + Integration v2 + Catalog Migration v2 + Backlog v11 + Layer 3 3A spec shipped). v6 closed 2026-05-13 (post-L3-Spec-Trio); v5 closed 2026-05-13 (FC-4b); v4 closed 2026-05-13 (FC-4a); v3 closed 2026-05-13 (FC-3); v2 closed 2026-05-12; v1 closed 2026-05-11.
...
1. **SQLite path retired in PR13 (2026-05-16, D-54 ✅ Resolved) — supersedes v6's "removed during Phase 5" framing.** §"What changed in v6 vs v5" item #7 (line 25 below) characterised the dual-backend collapse as "removed during Phase 5 of the catalog migration." That coupling was broken by PR13, which retired the SQLite path as a standalone stack-cleanup PR independent of the Catalog Migration Plan. See `Catalog_Migration_Plan_v3.md` §5 decision #5 ✅ Resolved + `Athlete_Data_Integration_Spec_v5.md` §2.5 [RETIRED 2026-05-16, PR13] + `V5_Implementation_PR13_Closing_Handoff_v1.md` + `V5_Implementation_PR14_Doc_Sweep_Closing_Handoff_v1.md` for the full retirement narrative. **Forward-looking force of v6 item #7 is zero;** the historical narrative remains preserved verbatim per Rule #12 because the "What changed in v6 vs v5" section is in-project history, not a present-state characterization. Any forward-looking reading of v7 §"What changed in v6 vs v5" item #7 should defer to the v8→v7 narrative here.
2. **TrueNAS Docker deployment retired in PR13 (D-65 ✅ Resolved).** Not referenced in v7's body, but worth flagging here for the same audit-trail reason: any future reader scanning Control_Spec for deployment-context references should know the TrueNAS / Docker path (`Dockerfile`, `docker-compose.yml`, `deploy/truenas_setup.sh`, `deploy/update.sh`) was retired in PR13 alongside SQLite. Vercel is the production deployment target.
3. **§9 doc map snapshot is stale; cross-references resolve to highest-N at use time (Rule #12).** Per the top-of-§9 framing, "Cross-references in this map use logical names without file-revision suffix; resolve to the highest-N version present in project knowledge." So the snapshot version pins are not load-bearing — readers should resolve via directory listing. For the record, between v7 (2026-05-14) and v8 (2026-05-16) the following cross-references advanced: `Athlete_Onboarding_Data_Spec` v4 → v5 (consolidates D-58 + D-59 + D-60 + D-61); `Athlete_Data_Integration_Spec` v2 → v5 (§2.5 SQLite freeze flagged Retired); `Catalog_Migration_Plan` v2 → v3 (SQLite retirement decoupled from §1/§5); `Project_Backlog` v11 → v29 (rolling tracker advanced through PR1–PR16); `Layer3_3B_Spec` ⏳ → ✅ shipped 2026-05-14 (goal-timeline viability + periodization shape; no §9 row in v7 because v7 listed it as ⏳ "next forward move"). New design-wave specs landed under the Cross-cutting category but are not currently listed in §9: `Onboarding_D58_Design_v1`, `Onboarding_D59_Design_v1`, `Onboarding_D60_Design_v1`, `Onboarding_D61_Design_v1`, `OnDemand_Workout_D63_Design_v1`, `Plan_Refresh_D64_Design_v1`. A full §9 doc-map sync is owed when Control_Spec gets its next architectural revision — same opportunistic-cleanup framing as the v7→v8 SQLite cleanup itself. **Not done in v8** to keep this revision tightly scoped to the SQLite-cleanup deferred item per PR15 handoff §5.1 Option B.
4. **No body changes.** §§1–8 and §§10–11 unchanged from v7. The architecture itself (PG-only, single Neon Postgres database with `layer0.*` + `public.*` schemas, Vercel deployment) is unchanged — PR13 stripped the dual-backend implementation but the architecture was always PG-target. The §9 doc map is left as v7 wrote it; the Rule #12 resolve-to-highest-N convention carries the load until the next §9 sync.
