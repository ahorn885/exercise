...
**Status:** v1 — 2026-05-12 (FC-2). v1 reflects FC-1b column promotions (D-22, D-23). First draft was 2026-05-10 per `Batch_C_Done_2D_Kickoff_Handoff.md`.
**Type:** Query node. Pure read, deterministic given inputs, no LLM involvement.
**Supersedes:** the stub `q_layer2d_injury_risk_profile_payload(disciplines, etl_version_set)` in `Layer0_ETL_Spec_v3.md` §5.2. That signature is unbuildable — 2D requires the athlete's injury and condition records as direct inputs. `Layer0_ETL_Spec_v4` folds in this signature.

---

## What changed in v1 vs unversioned draft

1. **§5.3.3 (Movement-constraint match) rewritten to set-intersect against deployed `exercises.movement_components TEXT[]`** — replaces heuristic keyword-match against `injury_flags_text`. D-22 closed (FC-1b, 2026-05-12); 159/159 exercise rows populated; canonical 11-token set defined by Onboarding §B.3. The in-code `MOVEMENT_CONSTRAINT_KEYWORDS` table is demoted to a historical reference and is no longer the runtime source. 2D-2 closed.
2. **§5.4 (`discipline_risk()` function) rewritten to set-intersect against deployed `disciplines.body_parts_at_risk TEXT[]`** — replaces patterns-text + keyword-fuzzy match. D-23 closed (FC-1b, 2026-05-12); 31/31 discipline rows populated; canonical 51-token body part vocabulary per `Vocabulary_Audit_v3` Section 1.
3. **§5.5 Decision Point B locked to Path B (structured column).** Path A (`BODY_PART_KEYWORDS` in-code map) demoted to historical reference only — kept inline so the v1→v4 transition is auditable, not as runtime code. 2D-1 closed.
4. **§6 drift items D-22 and D-23 marked Resolved.**
5. **§12 open items 2D-1, 2D-2, 2D-6 marked Closed.**
6. **D-38 (wrist deviation token gap)** noted as known sub-threshold limitation in §5.3.3 — 2 data points across 159 exercises, force-mapped to `Pain above specific joint angle` (Angle) token. Tracked for v2 vocabulary review.
7. **No other section changes.** §§1-4, §5.1, §5.2, §5.3.1-2, §5.3.4-5, §5.6, §5.7, §§7-11, §§13-14 unchanged from the unversioned draft.
...
The athlete's `InjuryRecord.movement_constraints` is a multi-select drawn from Onboarding §B.3's canonical 11-token enum. As of D-22 closure (FC-1b, 2026-05-12), exercises have a parallel deployed column `exercises.movement_components TEXT[]` populated from the same 11-token vocabulary. Match is now a direct set-intersect — no keyword fuzz against free text.

**Canonical token set (11):**

```
Pain with loading                  · Pain with impact
Pain above specific joint angle    · Pain on descent / eccentric
Pain on rotation                   · Pain with grip / sustained hold
Pain with wrist extension          · Pain with overhead movement
Instability                        · Reduced ROM
Pain at high volume only
```

```python
def movement_constraint_verdict(exercise, current_injuries) -> tuple[Verdict, list[Evidence]]:
    if not exercise.movement_components:
        # Defensive — D-22 migration populated all 159 active rows;
        # empty array indicates a future row that hasn't been curated yet.
        return Verdict.CLEAN, []

    components = set(exercise.movement_components)
    evidence = []
    verdict = Verdict.CLEAN
    for inj in current_injuries:
        intersect = components & set(inj.movement_constraints or [])
        if intersect:
            sev_verdict = severity_to_verdict(inj.severity)
            evidence.append(Evidence(
                source='movement_constraint',
                injury=inj,
                exercise_field='movement_components',
                matched_components=sorted(intersect)
            ))
            verdict = max(verdict, sev_verdict)
    return verdict, evidence
```

**Set-intersect mechanics in Postgres.** The deployed column has GIN index `idx_exercises_movement_components`. Query-layer code uses the `&&` overlap operator for filter pushdown:

```sql
SELECT exercise_id, movement_components
FROM layer0.exercises
WHERE superseded_at IS NULL
  AND movement_components && ARRAY[/* athlete's current movement_constraints */]::text[];
```

This is the canonical pattern for movement-constraint filtering in 2D's candidate-enumeration step (§5.2).

**Historical reference — pre-D-22 keyword-match map.** Until FC-1b, 2D used heuristic keyword matching against the free-text `injury_flags_text` column via an in-code `MOVEMENT_CONSTRAINT_KEYWORDS` table. That table is preserved below for audit history only — it is **not** consulted at runtime in v1 of this spec.

```python
# DEPRECATED — kept for v1→v4 transition audit. Do not call.
...
`injury_flags_text` is retained on `exercises` as reference data (for human review and Layer 3 reasoning), but `movement_components` is the source of truth for 2D filtering.

**Known sub-threshold limitation — D-38.** The 11 canonical tokens do not include wrist *deviation* (ulnar/radial); they cover wrist *extension* only (`Pain with wrist extension`). Two exercises in D-22 curation surfaced wrist-deviation stress (EX126 Freestyle Pull, EX235 Tricep Pushdown/Extension) and were force-mapped to `Pain above specific joint angle` (Angle) as the nearest canonical token. Two data points across 159 exercises is below threshold for a 12th canonical token; tracked for v2 vocabulary review. Documented in `D22_Curation_Reference_v2.md` §Calibration D / Pass 2 sections.
...
For each `discipline_id` in `included_discipline_ids`, evaluate body-part overlap against the athlete's current and resolved injury body parts using set-intersect on the deployed structured column.

**Source data (post-FC-1b):** `disciplines.body_parts_at_risk TEXT[]` — 31/31 rows populated against the canonical 51-token body part vocabulary from `Vocabulary_Audit_v3` Section 1. Free-text `common_injury_patterns` is retained as reference data for human review and Layer 3 narrative generation but is **not** the source of truth for 2D risk matching.

```python
def discipline_risk(discipline, injuries) -> DisciplineRisk:
    at_risk = set(discipline.body_parts_at_risk or [])
    if not at_risk:
        return DisciplineRisk(
            ...
            reasoning='No body-part risk data for this discipline.'
        )

    matched_current = []
    matched_history = []
    for inj in injuries:
        if inj.body_part in at_risk:
            target = matched_current if inj.severity != 'Resolved' else matched_history
            target.append(MatchedBodyPart(
                body_part=inj.body_part,
                side=inj.side,
                severity=inj.severity,
                matched_via='body_parts_at_risk'
            ))
...
**Set-intersect mechanics in Postgres.** The deployed column has GIN index `idx_disciplines_body_parts_at_risk`. For a batch query across all the athlete's `included_discipline_ids`, the canonical pattern is:

```sql
SELECT discipline_id, body_parts_at_risk
FROM layer0.disciplines
WHERE discipline_id = ANY(:included_discipline_ids)
  AND body_parts_at_risk && ARRAY[/* athlete's injury body_parts (current + history) */]::text[];
```

This returns only disciplines with overlap; non-overlap disciplines are LOW risk by §5.4's rubric and don't need row materialization.

**Risk-level rubric (unchanged from unversioned draft):**
...
**Vocabulary alignment.** Both the athlete's `InjuryRecord.body_part` field (Onboarding §B.1) and the deployed `body_parts_at_risk` column draw from the same canonical 51-token list. Set-intersect is therefore exact — no synonym mapping, no fuzz, no false negatives from anatomical-vs-clinical naming. Vocabulary maintenance happens in one place (`Vocabulary_Audit`), and adding a 52nd token (when needed) requires updating only the curation reference + a re-migration of `body_parts_at_risk`; the 2D code is unchanged.

### 5.5 Body-part vocabulary alignment (historical decision point — Path B selected)

**Status:** Locked. Path B selected. Deployed FC-1b 2026-05-12. Decision history retained below for audit; the runtime path is described in §5.4.

The athlete's structured `body_part` (e.g., `'Knee'`) must align with the discipline's per-discipline risk profile. In the unversioned draft of this spec, the discipline source was free-text `common_injury_patterns` (e.g., `'IT Band Syndrome · Plantar Fasciitis · Patellar Tendinopathy · Achilles Tendinopathy · Ankle sprains'`), which required either a synonym map (Path A) or a parallel structured column (Path B).

**Path A (rejected) — code-side hand-curated synonym map.** A `BODY_PART_KEYWORDS` table mapping each of the canonical 51 body parts to free-text synonyms found in `common_injury_patterns`. The match was a substring scan over the lower-cased text. This was the v1 recommendation in the unversioned draft. Reproduced below for audit only — **not used at runtime in v1 of this spec**.
...
**Path B (selected, deployed) — Layer 0 structured column.** `disciplines.body_parts_at_risk TEXT[]` is populated by hand-curation from `common_injury_patterns` text, drawing from the canonical 51-token body-part vocabulary. Curation reference: `D23_Curation_Reference_v1.md` (house rules 1–7, synonym table, inheritance log, 31 mappings). Migration: `migrate_disciplines_add_body_parts_at_risk_v1.sql`. GIN index: `idx_disciplines_body_parts_at_risk`. 172 total token references across 31 disciplines; 28 of the 51 canonical body parts are referenced by at least one discipline (the unused 23 cluster in body parts that don't currently appear in any discipline's risk profile — e.g., Jaw, Bicep, PCL — not because they're never injured but because no discipline's injury-pattern text flagged them).

**Why Path B won.**
- **Queryable and auditable.** `SELECT discipline_id, body_parts_at_risk FROM ...` returns the exact match surface. Path A's keyword fuzz hid the actual match logic in code.
- **No synonym drift.** When `Vocabulary_Audit_v3` added Collarbone (51st token), Path B required a one-row curation update (D-006 Mountain Biking) and a re-migration. Path A would have required a code edit to add Collarbone synonyms to `BODY_PART_KEYWORDS`.
- **Maintenance lives with vocabulary, not with code.** Future canonical-list changes touch curation references + migration generators only.
- **Removes false-negative risk from keyword fuzz.** Substring-scanning `'patellar tendinopathy'` for `'kneecap'` would have missed; Path B's structured `{Knee, Kneecap}` enumeration catches both explicitly per the curation house rules.

**Trade-off accepted.** Path B costs an upfront curation pass (now done) and any future maintenance when discipline injury patterns evolve. Both costs are bounded and concentrated — the curation pass took one session under the generator-script pattern, and future re-runs follow the same recipe. Tracked closure: 2D-1 in §12.

Per-curation-row decisions, scope inheritance (D-008b, D-009, D-016, D-021, D-022, D-025, D-026), and Shoulder/Rotator-cuff normalization calibration are documented in `D23_Curation_Reference_v1.md`.
...
| D-15 | `discipline_substitutes` UNIQUE constraint in deployed includes `substitute_name`, allowing in principle duplicate (target, substitute) with different names. Resolved with explanation in FC-1a: variant key preserved deliberately. | ✅ Resolved (FC-1a, 2026-05-11) |
| D-21 | **`health_condition_categories` column name uncertainty.** 2D matches against enum values, not column names (standing rule, FC-1a). Documentation cleanup deferred to FC-2 spec rewrite. | 🟡 Deferred (documentation) |
| D-22 | **`exercises.movement_components TEXT[]` deployed.** 2D §5.3.3 set-intersect path replaces heuristic keyword-match against `injury_flags_text`. 159/159 active rows populated; GIN index `idx_exercises_movement_components`. | ✅ Resolved (FC-1b, 2026-05-12) |
| D-23 | **`disciplines.body_parts_at_risk TEXT[]` deployed.** 2D §5.4 / §5.5 set-intersect path against canonical 51-body-part vocabulary replaces the in-code `BODY_PART_KEYWORDS` map. 31/31 disciplines populated; GIN index `idx_disciplines_body_parts_at_risk`. | ✅ Resolved (FC-1b, 2026-05-12) |
| D-38 | **Wrist deviation token gap.** Canonical 11-token movement-components set does not include ulnar/radial deviation. Two D-22 force-mappings to Angle (EX126, EX235). Sub-threshold for 12th token. | 🟢 Cleanup (v2 vocab review) |
...
| # | Item | Owner | Status |
|---|---|---|---|
| 2D-1 | **Body-part keyword map location** (§5.5 decision point) — code-side hand-curated map vs. `disciplines.body_parts_at_risk TEXT[]` column | Andy / FC | ✅ Closed (FC-1b, 2026-05-12) — Path B selected and deployed. See D-23 in §6. |
| 2D-2 | **Movement-constraint keyword map location** — code-side (B.3 enum mirror) vs. Layer 0 structured column | Andy / FC | ✅ Closed (FC-1b, 2026-05-12) — Layer 0 structured column `exercises.movement_components TEXT[]` deployed. See D-22 in §6. |
...
| 2D-5 | **`health_condition_categories` column name reconciliation** — spec v3 §4.14 references `category_name`; v3 §6.2 validation references `system_category`. Deployed name uncertain. Standing rule from FC-1a: 2D / 2E match on enum values, not column names. | FC-2 spec rewrite | Backlog item D-21 (documentation deferred) |
| 2D-6 | **`exercises.injury_flags_text` → structured `movement_components TEXT[]`** — referenced as cross-layer enhancement in `Athlete_Onboarding_Data_Spec_v2.md` §B.3 | Closed (FC-1b) | ✅ Closed (FC-1b, 2026-05-12) — duplicate of 2D-2. `injury_flags_text` retained as reference data; `movement_components` is source of truth. |
...
| 2D-11 | **Wrist deviation canonical token gap (D-38).** 11-token movement-components set lacks ulnar/radial deviation. Two force-mappings to Angle in D-22 curation (EX126, EX235). | v2 vocabulary | Sub-threshold for v1; track for next vocab review |
