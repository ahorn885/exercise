...
**Status:** Draft v1, 2026-05-16. Contract sections (§§1–13) complete; §14 retrospective deferred to a follow-on session with fresh eyes (§12.6). Session 1 covered §§1–3 + §7 Payload schema (including `RaceWeekBrief` + `RacePlan`). Session 2 added §§4–6 + Decision 8 (seam-reviewer authority = β propose-patch). Session 3 covered §§8–9 (coaching flag rules + caching/determinism) + 2 calibration rounds resolving all session-1/2/3 policy-default backlog. Session 4 drafted §§10/11/13 (edge cases + performance budget + test scenarios) including concrete detection algorithms for the four `Layer4ShapeInfeasibleError` classes. **Session-4 close pass (this update): §12 (open items / forward references) drafted with 6 subsections — resolved items, prompt-body forward-pointers, downstream-spec forward-pointers, tuning candidates, substantive direction holds, §14 retro deferral; §14 stub re-labeled to reflect intentional deferral.** Per Andy 2026-05-16: ship the v1 spec arc as a PR + merge now (per the "push to production as we go" rule); §14 retro lands before Layer 4 implementation. No new source decisions this update. **Amendment 2026-06-21 (Andy, #214): §10.2 shape-infeasibility re-scoped.** The four-detector design was cut to one blocker (`cumulative_load_injury_infeasible`) + one warning (`schedule_volume_under_target`); discipline-frequency + skill-acquisition detectors deleted. Detection + athlete resolution moved to the new **Layer 3D HITL gate** (`Layer3D_Spec.md`); Layer 4 keeps a defensive raise only. The open `Layer4ShapeInfeasibleError` routing question (§12.3 finding C3) is resolved → 3D gate. Touched: §3.5, §6.4, §10.2, §10.10, §12.3/§12.4, §13 (TS-10–13, TS-75), §14.
...
- `Layer4ShapeInfeasibleError(class='cumulative_load_injury_infeasible', evidence)` — **defensive only** (2026-06-21 re-scope, §10.2). 2D exclusions empty a phase's strength pool or ban a discipline's only cardio modality. Primary detection + athlete resolution is owned by the **3D gate** (`Layer3D_Spec.md`); Layer 4 raises this only if synthesis is reached on the infeasible shape anyway. The former schedule-volume / discipline-frequency / skill-acquisition classes are removed (demoted to a 3D warning / deleted, respectively).
...
- **>2-training/day clamp (#351, ratified 2026-06-21).** The daily cap is on TRAINING sessions only (`kind ∈ {'cardio','strength'}`; recovery is exempt and additive — never counted or trimmed). When a synthesizer over-emits **more than 2 training sessions on one calendar day**, Layer 4 deterministically clamps that day to 2 **before** payload construction (`per_phase._clamp_sessions_per_day`) rather than relying on the retry path — this is an intentional rule, not a hidden trim: the over-emitted session cannot be placed within the 2-training/day invariant, so it is dropped. **Drop policy:** KEEP a `cardio` + one other training session (preferring the non-`hard`, lowest-`session_index_in_day` sessions; the kept cardio guarantees the "at least one of two must be `cardio`" rule above up front), and discard the rest. A day with no `cardio` at all (all-strength over-emit) keeps its two lowest-index strengths and the deterministic strength-collision repair resolves the surviving pair. Surviving sessions on a clamped day are renumbered to a contiguous `session_index_in_day` 0..n-1. The clamp runs ahead of the strength-collision / day-composition repairs (which assume ≤2 training/day); any residual two-hard or strength+strength on the kept pair is healed there. The #349 `ValidationError → schema_violation` retry wrap remains the safety net for the rarer cross-session invariants (e.g. `>1 recovery/day`) that are not deterministically clamped.
...
| Discipline inclusion | `discipline_excluded_*` | Per session | `discipline_id` is in 2A `discipline_inclusion`. Catches stale prior-plan sessions referencing a retired discipline. Severity: `blocker`. **D-67 per-date discipline exclusion dropped (Andy, 2026-06-30):** discipline availability is governed by the gear/terrain availability rules instead; no per-date exclusion restriction exists. |
| Sport-locale compatibility | `sport_locale_incompatible_*` | Per **strength** session | Advisory only (`warning`). 2C `discipline_coverage` measures how many of a discipline's *strength-conditioning* exercises (Layer 0B) resolve against a locale's equipment — it is advisory per `Layer2C_Spec §8.1`, not a gate. Fires only when a **strength** session's `discipline_id` has zero resolvable coverage at `locale_id`; the hard per-exercise equipment gate is Rule 6a `equipment_unavailable`. **Cardio/sport sessions are NOT evaluated here** — sport-at-locale trainability is a *terrain* question (Layer 2B, non-gating; multi-locale cluster work), not equipment-coverage. (Prior behavior hard-blocked sport sessions on strength-coverage — a category error that stalled cold plans on outdoor disciplines with no 0B exercises, e.g. MTB / Rock Climbing; corrected to advisory + strength-scoped.) |
...
- **D-67** per-date athlete restrictions — **✅ shipped 2026-06-30 (#237)**, scope reduced to **locale lock + indoor-only** only (gates rules `session_locale_not_in_cluster_*` (6c) and `indoor_only_violation_*`). The originally-specced `discipline_exclusions` and `max_total_minutes` restriction fields were **dropped** (Andy, 2026-06-30) before any UI ever captured them: discipline availability is governed by the gear/terrain availability rules instead, and the per-day `volume_by_date` percentage (#889) already supersedes a minutes cap. `discipline_excluded_*` and `daily_window_fit_*` no longer carry D-67-extension branches.
...
`session_locale_not_in_cluster_*` and `indoor_only_violation_*` ship D-67-aware branches in the v1 validator harness; they no-op when the data isn't present (defaults to empty restriction set per date).
...
- **Schema-violation special case.** When the synthesizer returns malformed structured output (per output parser): one schema-validation retry (counter does NOT consume the per-phase budget — schema retries are separate); on second failure, raise `Layer4OutputError('schema_violation')` and bail out of the call.
  ...
  > **#47 amendment 2026-05-31 (top-level payload invariants are schema-violations too).** "Malformed structured output" is hereby defined to include **both** layers of validation, not just the per-row output parse: (a) per-`PlanSession` field/parse failures (caught when building each row), **and** (b) **top-level `Layer4Payload` `@model_validator` failures** that only fire at payload *construction* — the cross-session invariants in §7.12: `_check_two_per_day` (>2 sessions/date; no strength+strength; no two `hard`; ≥1 `cardio` of a pair; two-a-day `session_index_in_day ∈ {0,1}`), `_check_mode_invariants`, `_check_validator_results`, `_check_shape_override_observation`, `_check_phase_metadata_per_mode`. Both are retried under the same schema-retry path and, on persistent failure, raise `Layer4OutputError('schema_violation')` (a `_RETRYABLE_BLOCK_CODES` member — the block re-synthesizes on the next resumable pass, bounded by the §11.5 stall backstop). **Rationale:** category (b) used to be raised as a raw `pydantic.ValidationError` from `_build_payload_for_validation`, which sat *outside* the per-row parse `try/except` — so it escaped the layer's typed-error contract, hit the route catch-all, and discarded the **whole** plan as "failed unexpectedly" over a single bad block (prod plan #47, 2026-05-31: the synthesizer over-emitted 3 sessions on 2026-06-03; "max 2 sessions per day (got 3)"). Wrapping the construction makes (b) behave like (a) — one bad block is retried, not fatal. *(The common >2/day case is a candidate for a pre-validation per-day clamp so it self-heals without spending a retry — tracked as a follow-up; the retry path is the safety net.)*
...
**Infeasibility cases that do NOT shape_override — handled by the 3D gate:**

The athlete is making a real choice (more days, drop a discipline, relax an injury input) that Layer 4 should not silently rearrange. Per the **2026-06-21 re-scope** (see §10.2), these are owned by the **Layer 3D HITL gate** (`Layer3D_Spec.md`), which runs before Layer 4 — not by a Layer-4 escalation. Only the injury case remains a Layer-4-defensive `Layer4ShapeInfeasibleError`.

| Class | Disposition (per §10.2) |
|---|---|
| Schedule-volume under target | **Warning, not a blocker.** 3D emits `schedule_volume_under_target`; Layer 4 clamps prescribed volume to available capacity and proceeds. Athlete acknowledges on the review screen. |
| Discipline-frequency | **Deleted** — not every discipline trains every week (Andy 2026-06-21). |
| Skill-acquisition | **Deleted** — out of scope; we don't train skills (Andy 2026-06-21). |
| Cumulative load + active injury | **Blocker.** Detected by 3D (`injury_pool_empty`); plan parks at `needs_review`. Layer 4 keeps a defensive `Layer4ShapeInfeasibleError(class='cumulative_load_injury_infeasible')` raise only. |
...
### 10.2 Shape-infeasibility detection — moved to the 3D gate (2026-06-21 revision)

> **Amendment 2026-06-21 (Andy).** The original four-detector design was re-scoped. The detection that *survives* now lives in the **Layer 3D HITL gate** (`Layer3D_Spec.md` §5.2/§5.3), which runs after 3B and before Layer 4 — the gate is the natural home for "stop before synthesis," it already has the full 2A/2C/2D/3B input set the detectors need, and it is where the athlete resolves the finding. Layer 4 keeps **no detector of its own** — the 3D gate is the sole guard (as-built note below). Three of the four original classes were cut or demoted:
>
> | Original class | Disposition | Rationale (Andy 2026-06-21) |
> |---|---|---|
> | `schedule_volume_infeasible` | **Demoted to a warning** (no longer blocks). 3D emits `schedule_volume_under_target`; Layer 4's volume math already **clamps prescribed volume to available capacity** (`min(capacity, band)` in `phase_volume_bands_hours`), so the plan is trimmed to the athlete's real hours and proceeds. The athlete is *told* it may under-prepare them; they acknowledge and continue. | "Not enough hours should be a warning, not a blocker — trim to the hours the athlete has and tell them it's probably not enough, but allow it through." |
> | `discipline_frequency_infeasible` | **Deleted.** | "Not every discipline needs to be trained every week. That's not a valid reason to fail or block a plan." |
> | `skill_acquisition_infeasible` | **Deleted.** | "We don't train skills — not our problem, not something we should worry about." |
> | `cumulative_load_injury_infeasible` | **Kept — the one real blocker.** Detected + gated by 3D (`injury_pool_empty`); the 3D gate is the **sole guard** (no Layer 4 raise — see the as-built note below). | "This one is valid." |

**The surviving blocker (`injury_pool_empty` / `cumulative_load_injury_infeasible`).** After applying 2D exclusions to each phase's exercise pool (2C `effective_pool` minus 2D `excluded_exercises`): if a phase that needs strength work has `< 3` distinct usable strength exercises (v1 floor, Andy 2026-06-21), OR 2D excludes the only cardio modality for an included discipline, the shape is infeasible. **Detection authority is 3D** (`Layer3D_Spec.md` §5.2) — it raises a `blocker` gate item and parks the plan at `needs_review`; Layer 4 is never invoked. **As-built (2026-06-22, Andy — Layer 3D Slice 2): the 3D gate is the sole guard; Layer 4 adds no defensive raise.** The orchestrator evaluates the gate and raises `Layer3DGateBlocked` *before* Layer 4 synthesis (Layer 3D Slice 1), so a raise inside `orchestrate_plan_create` would be unreachable dead code; and a raise inside the *shared* synthesizer (`per_phase.synthesize_phase` — also used by `single_session` / `plan_refresh` / `race_week_brief`, none of which run the gate) would wrongly block an ad-hoc single session or a refresh that legitimately tolerates an empty plan-wide strength pool. `Layer4ShapeInfeasibleError` was never implemented (spec-only) and is not added; if a belt-and-suspenders raise is ever wanted, it must be scoped to the plan-create synthesis path only.

**The warning (`schedule_volume_under_target`).** Owned entirely by 3D (`Layer3D_Spec.md` §5.3). Not a Layer 4 error; Layer 4 just clamps and synthesizes as it already does. Layer 4 raises nothing for schedule volume.

**Routing — RESOLVED (2026-06-21).** The previously-open "inline error vs 3D gate" routing question (§12.3, finding C3) is closed: feasibility findings route to the **3D gate** (`Layer3D_Spec.md`). The gate surfaces them on an athlete-facing review screen with acknowledge (warnings) / revise (blockers) affordances before plan generation runs.
...
| Case | Trigger | Spec contract | Expected behavior |
|---|---|---|---|
| All days marked unavailable per §K | Athlete has `available=False` on every day of the week | §4.1, §10.2, `Layer3D_Spec.md` §5.3 | **2026-06-21 re-scope:** 0 available hours is below every phase target → 3D emits a `schedule_volume_under_target` **warning** (not a blocker). Layer 4 clamps to zero-capacity bands and synthesizes a minimal/rest-dominant plan; the athlete is told on the review screen it under-prepares them. (A genuinely empty schedule is the athlete's choice to acknowledge, not a Layer-4 hard stop.) |
...
- **`Layer4ShapeInfeasibleError` routing to athlete surface** — **RESOLVED 2026-06-21**: routes to the 3D HITL gate (`Layer3D_Spec.md`), surfaced on an athlete review screen before plan generation. Detection moved to 3D; Layer 4 keeps a defensive raise only (§10.2 amendment).
...
**Shape-infeasibility detection (§10.2 — re-scoped 2026-06-21; detection now owned by `Layer3D_Spec.md`):**

- `cumulative_load_injury_infeasible` (injury empties a phase's strength pool / bans a discipline's only cardio modality): the `< 3` usable-strength-exercises floor (Andy 2026-06-21). The one surviving blocker; tune in 3D.
- `schedule_volume_under_target`: the warning threshold (available weekly hours vs phase band low) lives in 3D (`Layer3D_Spec.md` §5.3). The former `0.85 × phase_load_bands.low` blocker floor is retired — no blocker.
- `discipline_frequency_infeasible` + `skill_acquisition_infeasible`: **deleted** (no longer detectors).
...
| **TS-10** | §K availability: athlete has only 3 days/week; 2A Build band targets ~10 h for the dominant discipline → schedule volume below target | **2026-06-21 re-scope:** no Layer 4 raise. 3D emits a `schedule_volume_under_target` **warning** (`Layer3D_Spec.md` §5.3); Layer 4 clamps volume to the available hours and synthesizes; athlete acknowledges on the review screen | §10.2, `Layer3D_Spec.md` §5.3 |
| ~~**TS-11**~~ | ~~discipline_frequency_infeasible~~ | **Deleted (2026-06-21)** — not every discipline trains every week; never a block | §10.2 |
| ~~**TS-12**~~ | ~~skill_acquisition_infeasible~~ | **Deleted (2026-06-21)** — out of scope; we don't train skills | §10.2 |
| **TS-13** | 2D excludes every available compound lift (severe shoulder + wrist injury); a phase's strength sessions have no usable pool | Detected by **3D** (`injury_pool_empty` blocker, `Layer3D_Spec.md` §5.2) — plan parks at `needs_review`, Layer 4 not invoked. Layer 4 retains a defensive `Layer4ShapeInfeasibleError(class='cumulative_load_injury_infeasible', ...)` raise if synthesis is reached anyway | §10.2, `Layer3D_Spec.md` §5.2 |
...
|---|---|---|
| **TS-75** | All §K days marked unavailable → `schedule_volume_under_target` **warning** (3D), not a Layer-4 raise; plan clamps + proceeds (2026-06-21 re-scope) | §10.10, §10.2, `Layer3D_Spec.md` §5.3 |
...
| C3 | **`Layer4ShapeInfeasibleError` routing — RESOLVED (2026-06-21).** The open "inline error vs 3D gate" decision is closed: feasibility findings route to the **3D HITL gate** (`Layer3D_Spec.md`), which surfaces them on an athlete review screen (acknowledge warnings / revise blockers) before plan generation. Detection moved to 3D; Layer 4 keeps a defensive raise only (§10.2). The schedule-volume class became a 3D warning; discipline-frequency + skill-acquisition were deleted. | No longer open. |
...
- **`Layer4ShapeInfeasibleError` routing** — **RESOLVED 2026-06-21**: routes to the **3D HITL gate** (`Layer3D_Spec.md`), not an inline error. Detection moved to 3D; Layer 4 keeps a defensive raise only. Schedule-volume → 3D warning; discipline-frequency + skill-acquisition deleted (§10.2 amendment).
