...
> | `cumulative_load_injury_infeasible` | **Kept — the one real blocker.** Detected by 3D (`injury_pool_empty`); Layer 4 retains a defensive `Layer4ShapeInfeasibleError` raise for the same condition. | "This one is valid." |

**The surviving blocker (`injury_pool_empty` / `cumulative_load_injury_infeasible`).** After applying 2D exclusions to each phase's exercise pool (2C `effective_pool` minus 2D `excluded_exercises`): if a phase that needs strength work has `< 3` distinct usable strength exercises (v1 floor, Andy 2026-06-21), OR 2D excludes the only cardio modality for an included discipline, the shape is infeasible. **Detection authority is 3D** (`Layer3D_Spec.md` §5.2) — it raises a `blocker` gate item and parks the plan at `needs_review`; Layer 4 is never invoked. Layer 4 retains a **defensive** check that raises `Layer4ShapeInfeasibleError(class='cumulative_load_injury_infeasible', evidence)` if synthesis is somehow reached on this shape anyway (mirrors the §4 "caller pre-checks; Layer 4 raises defensively" pattern). `evidence` carries the phase/discipline, the post-exclusion usable-exercise count, and the triggering 2D exclusion ids. When raised, no Layer 4 sessions are written; the `plan_versions` row is rolled back per D-64 §6.2 atomic-write semantics.
