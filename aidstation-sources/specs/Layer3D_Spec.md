...
Per phase: compute the athlete's bounded available weekly hours (`weekly_capacity_hours(layer1_payload)` — Σ enabled §K daily windows, capped by `weekly_hours_target`) and the phase's target low band (`phase_volume_bands_hours(...)` low edge for the dominant discipline). If available `< target_low`, emit **one** warning item (not per-phase spam — the worst/most-constrained phase carries it, with the others listed in `evidence`).
