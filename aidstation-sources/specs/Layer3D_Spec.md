...
| Plan is clean except for surfaced advisory `coaching_flags` | `gate_status='green'`; orchestrator advances to synthesis. Informational items are display-only (§5.4) — they don't park the plan; they ride the review screen only when it's shown for another reason, and list on the plan home as coaching notes (Item B, §11). |
...
**Green-plan coaching notes (Item B, §5.4).** The review screen only appears when a plan parks, so a fully-green plan never showed its `informational` items. The plan home (`view_plan` → `templates/plan_create/view.html`) lists them as **coaching notes** via `_plan_coaching_notes(db, uid, plan_version_id)` — the persisted gate's `informational` items (the surfaced advisory `coaching_flags` + any 3B informational note), rendered as display-only prose. Nothing here gates; it's the cross-node FYI context (e.g. an included discipline gear-gated at every locale) the athlete should see even on a clean plan. Advisory/fail-safe: a gate load fault degrades to no notes, never 500s the plan view (mirrors the nutrition/conditions/evidence loads in the same view).
...
| TS-3C-10 (Item B, plan-home notes) | A generated (green) plan whose gate carries `informational` items | `_plan_coaching_notes` returns only those items; `view.html` lists their messages as coaching notes (warnings/blockers never render there); a gate load fault degrades to no notes, never 500s the view |
