...
**Status:** v1, 2026-06-22. **Slices 1 + 2 + 3 implemented** (`layer3d/gate.py` + `routes/plan_create.py` + `layer4/orchestrator.py`): Slice 1 = aggregation + gate (PR #850); Slice 2 = the two §5.2/§5.3 feasibility detectors (#868); Slice 3 = the §11 revise cascade + §11.2 staleness re-fire ([Fix this] → Layer-1 edit surface → re-gate via `orchestrate_plan_create(gate_only=True)`, which re-parks even when green so the athlete still clicks [Generate plan]). 3C cross-node conflict (§13) is the one remaining slice. Scopes the 3D human-review gate that sits between Layer 3B and Layer 4: it collects the human-review items the upstream nodes already emit (and that are currently produced + silently discarded), runs two pre-synthesis feasibility detectors, and **gates Layer 4** — plan generation runs only when every item is resolved. Per `Control_Spec_v8.md` §2 (Layer 3 — 4 nodes) + §7 (HITL surface). The `.5` resolution gate collapses into this node per Control_Spec §2 (no standalone row).
...
| `evaluated_against` | `dict[str,str]` | the `etl_version_set` the gate was computed against (staleness guard) |
...
> **As-built (2026-06-22, Slice 3).** `[Fix this]` POSTs `kind=revise` to `/review/resolve`, which records a `revised` `GateResolution`, sets `Layer3DGate.stale=True` (persisted in the `hitl_gate` JSONB — no schema column), and redirects to the Layer-1 edit surface (`plan_create._REVISE_TARGET_ENDPOINTS`). On return to the review screen, a stale gate triggers a **re-gate**: `orchestrate_plan_create(gate_only=True)` re-runs the upstream cone (the injury/equipment/discipline layers are deterministic DB queries, so they reflect the edit; the LLM layers hit cache) and re-aggregates, then ALWAYS re-parks — even when green — so the athlete still clicks [Generate plan] (§11 step 3). Resolutions survive by `item_key` (`load_prior_resolutions`). The re-gate runs through the existing advance loop (`_regate_parked_plan`, under the advance lock) — never synthesizing. The `[Generate plan]` click is the universal staleness backstop (it runs the full `orchestrate_plan_create`, which re-evaluates the gate before building). **No eviction wiring was needed** — 2D is a deterministic query and its hash folds into the Layer-4 synthesis cache key (`hashing.py` `layer2d_hash`), so a fixed injury both re-aggregates in the gate and yields a fresh plan on generate. **v1 scope (named in §13):** the stale flag is set on an athlete `[Fix this]` edit; a provider-sync-while-parked that shifts 3A→3B is caught by the generate-click backstop rather than auto-re-firing the idle review screen.
...
- **Revise-cascade wiring depth — DONE (Slice 3; §11.2 as-built).** `[Fix this]` routes to the Layer-1 edit surface; the return re-gates via `orchestrate_plan_create(gate_only=True)` (the cone's deterministic 2D/2C/2A reflect the edit; LLM layers hit cache). Remaining v1 gap: a provider-sync-while-parked (3A→3B shift) does not auto-re-fire the idle review screen — the generate-click backstop catches it. A push-based re-fire (provider sync marks the parked gate stale) is a future refinement.
- **Layer 4 defensive raise — RESOLVED (Andy 2026-06-22: no raise).** `Layer4ShapeInfeasibleError` was never implemented and is not added; the **3D gate is the sole guard** (the orchestrator raises `Layer3DGateBlocked` pre-synthesis, and a raise on the shared synthesizer would wrongly block single-session / refresh / race-week). See `Layer4_Spec.md` §10.2 as-built. The schedule/frequency/skill classes remain removed.
