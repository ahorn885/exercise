...
7. **Integration bundle → summary table.** Per accessor (recent_workouts, recent_wellness, recent_self_report_sleep, combined_load, connected_providers): record counts, date ranges, per-source breakdown. `recent_wellness` is one coalesced row per calendar day — sleep duration, overnight HRV, and resting HR each resolved by freshest-non-null across device providers (garmin/polar/coros/whoop) and rendered as latest-value + provenance + window-average per metric (a NULL or older source never clobbers a populated or newer one; ties break garmin>whoop>polar>coros). Whoop (`provider_raw_record` daily summary from a manual `physiological_cycles.csv` upload — #767 slice 4) is the first non-Garmin resting-HR source. Self-report sleep rides separately in `recent_self_report_sleep` so §6.1's objective-vs-subjective weighting stays intact. ACWR per-discipline shown as ratio + risk zone (per `q_layer3A_combined_load` output).
