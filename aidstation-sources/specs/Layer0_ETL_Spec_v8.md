...
> **⚠ Authoring model retired (epic [#488](https://github.com/ahorn885/exercise/issues/488), 2026-06-11).** The `layer0.*` DB tables are now the source of truth. Layer 0 reference data is authored as reviewed SQL migrations under `etl/migrations/layer0/`, validated by the `validate_layer0` gate — **not** by editing the xlsx workbooks and re-running the ETL. The extractor / `run.py` / `emit_sql.py` toolchain and the source workbooks are **frozen** under `etl/_frozen_xlsx_authoring/`; do not revive them. The current authoring model is `designs/Layer0_AuthoringModel_DBSourceOfTruth_Design_v1.md`.
>
> This spec is retained as the canonical **schema / table / validation reference** (§4 table definitions, §6 validation logic — still current). Its §3–§6 *extraction & ETL-run* mechanics describe the now-retired xlsx pipeline and are **historical**; read them for how the genesis data was produced, not as instructions to author new data.
