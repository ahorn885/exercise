...
**Status:** **SLICE 6a (PLAN-GEN MECHANICS) BUILT 2026-06-21** (branch `claude/593-volume-windows-build`; full suite 3213 passed / 30 skipped, etl 91). The deterministic capacity-reduction path is live end-to-end: repo (types + `volume_pct`), `init_db` ALTER (auto-applies on deploy — public-schema `_PG_MIGRATIONS`, no manual Neon apply), `session_feasibility` (override/segment volume fields), `orchestrator` (segment emission), `session_grid` (`volume_capacity_factor`), `per_phase` (per-week factor + overlay directive), `hashing`. **SLICE 6b (capture UI) BUILT 2026-06-21** (branch `claude/593-volume-windows-ui-6b`): the two constraint options + a `volume_pct` percent select on the existing event-window form (`routes/profile.py` parses percent → (0,1) fraction; `templates/profile/event_windows.html` adds the options + control + list labels; no inline JS per CSP). Full suite 3215 passed. **Owed sign-off:** the Trigger-#1 overlay wording (drafted in `per_phase._event_window_label` / `_format_event_window_overlay`, easily revised). Resolution model settled 2026-06-21: a single **deterministic capacity reduction** feeding the existing grid — explicitly *not* an LLM-reallocation / trim-to-fit cascade (Andy: that "introduces multiple new points of failure with the LLM looking for days or time slots it can't find").
**Date:** 2026-06-21
