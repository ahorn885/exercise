...
**Andy's reframe (2026-06-14) widens it further:** the thing we're missing isn't "travel" — it's **event windows**: date-bounded periods where the athlete's training environment differs from their default. Two kinds:
- **Home-constraint windows** — the athlete is *home* but can't train normally: weather, or (Andy's case) kids over who need supervision → **no outdoor cardio / indoor-only** for those days. (v1 had exactly this via `plan_travel.indoor_only`.)
- **Away windows** — the athlete is at a *different location* for the window (work trip, training camp), with that location's terrain/equipment/craft.
...
An **event window** = `{date range [start, end], kind, optional location, optional constraints}`:
- **kind = `home_constraint`** → no location change; carries a constraint (e.g. `indoor_only` / `no_outdoor_cardio`). Applies to the home environment for those dates.
- **kind = `away`** → a destination location (a `locale_profiles` row, reused or newly created inline) + optional brought-craft + optional assumed-vs-logged equipment.
...
- Location **categories** (hotel, airport, partner-gym, home, …) carry a **default assumed equipment profile** — reference data. A new away locale of category `hotel` inherits the hotel baseline until the athlete logs actuals.
- This usefully **resurrects the old `home/hotel/partner/airport` taxonomy** — not as the location's *identity* (WS-B retired that), but as **equipment-baseline templates**.
- **Trigger #2:** the baseline *contents* (which equipment each category assumes) are reference data needing Andy's ratification — a small Layer-0-style table (`location_category_equipment_baseline`) or seed. **Do not author the lists without sign-off.**
- Crowd-sourcing tie-in: assumed baseline → athlete-logged actual → shared `gym_profiles` (F2 dedup). The baseline is the cold-start; the crowd is the warm-start.

### F9 — Home-constraint windows (NEW, Andy) — the simplest vertical
A `home_constraint` window (no location change) carrying e.g. `indoor_only` / `no_outdoor_cardio`. For those dates, the **home** environment is resolved with outdoor terrain suppressed → outdoor cardio resolves to indoor/strength. This is the **smallest end-to-end slice** (no new location, no craft) and it exercises the full date-range + per-day feasibility machinery — so it's **Slice 1** (§6).
...
1. **Build the environment set** for the plan span: home cluster (default) + each distinct event-window environment (an away locale's terrain/equipment, or home-with-constraint).
2. **Resolve `_build_terrain_feasibility` per environment** (it already takes a locale set + per-locale maps; add the home-constraint variant = home maps with outdoor terrain suppressed; add the per-environment `owned_crafts`, F4).
...
- **Slice 1 — home-constraint windows (F9):** `athlete_event_windows` (date range + `home_constraint` kind + `indoor_only`/`no_outdoor_cardio`) + per-environment resolution with outdoor suppression + per-(week,env) feasibility feed + cache hash + a minimal capture/review panel. **Smallest end-to-end vertical; no location/craft/baseline.** Proves the date-range + per-day machinery on Andy's kids/weather case. DDL: `athlete_event_windows`.
...
- `athlete_event_windows(id, user_id, start_date, end_date, kind, location_locale|null, indoor_only, no_outdoor_cardio, notes, created_at)` — Slice 1 (+ `location` link in Slice 2).
- away discriminator on `locale_profiles` (e.g. `source`/`kind`) — Slice 2.
- `location_category_equipment_baseline(category, equipment_tags[])` reference data — Slice 3 (Trigger #2).
...
3. (Slice 1) Outdoor-cardio week with a 1-day home constraint → synthesis places the outdoor session on a non-constrained day (no substitution needed).
...
**Ratified (2026-06-14):** F1, F2, F4; reframe to event windows; F3 per-day; F8/F9/F-race direction.
**Owed before Slice-1 build:** (1) the F3 grid↔synthesis seam contract (weekly counts + per-day placement — the main risk; spec it precisely in Slice 1); (2) the `home_constraint` flag set (just `indoor_only`, or also `no_outdoor_cardio`/`reduced_volume`?); (3) Trigger-#1 feasibility-block wording (build time). **Per-slice triggers:** Slice 3 = Trigger #2 (baseline contents); Slices 1-4 DDL = owed Andy's hands.
...
- **Trigger #2 watch (F8):** don't author hotel/airport baselines without sign-off — that's the no-padding rule.
- **Race split is correct** — a race location is a target, not a training environment; conflating them would muddy both. Separate issue keeps the LLM-terrain-inference idea alive without bloating this arc.
- **What might be missing:** travel *days themselves* (in transit — partial/no training) — likely a `home_constraint`-like "reduced/none" window; note for Slice 1's flag-set decision (item 2 above).
