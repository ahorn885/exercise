# Modality Group — Spec v2

**Date:** 2026-06-12
**Author:** Claude + Andy
**Status:** DRAFT — design pending sign-off before implementation
**Supersedes:** `Modality_Group_Spec_v1.md` (additive clarification only — §3.3 records the second, Layer-4 feasibility use of `group_kind`; §12 forward-ref. No change to the pooling algorithm.)
...
**Second use — Layer 4 craft-feasibility axis (#540 slice 2c.2c, added v2).** `group_kind` has one additional, *downstream* consumer that does NOT touch this spec's algorithm: Layer 4's session-feasibility resolver (`layer4/session_feasibility.py:resolve_craft_feasibility`). When the athlete owns no craft for an included craft-bearing discipline (the `bike` / `paddle` kinds), the resolver substitutes the **session to actually do**, not the discipline's weight:

- owns a craft that trains the discipline's `group_id` (a gravel bike for MTB — both `bike_offroad`) → train as-is;
- owns only a same-`group_kind` craft in a *different* `group_id` (a road bike for MTB — `bike_pavement` vs `bike_offroad`) → **swap** the session to the owned craft's discipline (train the MTB allocation as a road ride), then the terrain axis resolves that discipline's surface;
- owns no craft of the kind at all → **strength** substitution.

This is a distinct concern from pooling and the two never conflict: **pooling (`group_id`) redistributes a race's WEIGHT** within a modality group at Layer 2A; the **craft axis (`group_kind`) chooses the substitute SESSION** at Layer 4 once weights are fixed. The §2 "does NOT do" boundary still holds — group membership never changes a discipline's *allocation*; the craft axis only changes how an already-allocated session is *composed* when the vehicle to do it isn't owned.
...
2. **Craft-feasibility axis (resolved, #540 slice 2c.2c).** The second use of `group_kind` — Layer 4's owned→swap→strength substitute ladder — is specified in §3.3 and built in `layer4/session_feasibility.py` + `layer4/orchestrator.py`. Recorded here so the "analytics-only" framing in §3.3 isn't read as excluding it.
