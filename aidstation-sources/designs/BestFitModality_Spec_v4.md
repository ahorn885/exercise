...
| **R6** | Two ID collapses (kayak D-008a/b, mtn-running D-022/D-023) + clean renumber | **✅ SHIPPED 2026-05-25 (own session)** | Full R6: clean sequential renumber → D-001..D-029 + both collapses (→ D-010 "Kayaking", D-024 "Mountain Running"). NOTE: the collapse flattened injury(2D)/stimulus(L3)/skill-gating(2A) data the terrain re-model never absorbed — the surface map proved "terrain axis consumed = safe to collapse" was incomplete; Andy ratified the collapse anyway (whitewater conditional retired = accepted regression). Authoritative old→new map + collapse-union spec + owed-Neon steps: `Discipline_ID_Renumber_R6_Design_v1.md`. |
