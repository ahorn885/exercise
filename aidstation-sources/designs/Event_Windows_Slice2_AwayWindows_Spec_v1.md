...
**Status:** **Slice 2a BUILT + MERGED-PENDING (2026-06-14, PR #600; full suite 2432 passed / 30 skipped).** Whole-week counts-follow-away threshold confirmed (Andy). Trips **Trigger #1** (away-overlay wording — **APPROVED** §7) + **Trigger #3** (one `away_locale` ALTER — **owed Andy's hands on Neon before/at merge**; `load_event_windows` SELECTs it). Slice 2b (inline-create UX) is the follow-up. §5 surface held as built (5 substantive core + thin hashing/route/template/DDL).
