...
**Status:** **Slice 2a BUILT + MERGED + LIVE (2026-06-14, PR #600 squash-merged; full suite 2432 passed / 30 skipped; `away_locale` ALTER applied on Neon by Andy).** Whole-week counts-follow-away threshold confirmed (Andy). Trips **Trigger #1** (away-overlay wording — **APPROVED** §7) + **Trigger #3** (one `away_locale` ALTER — **applied**). §5 surface held as built (5 substantive core + thin hashing/route/template/DDL). Handoff: `handoffs/V5_Implementation_WSH_EventWindows_Slice2a_AwayWindows_2026_06_14_Closing_Handoff_v1.md`. **Slice 2b (inline-create UX) BUILT + MERGED 2026-06-14** (PR #601 squash-merged to `main`, CI green; branch `claude/eventwindows-away-windows-xon8mb`; handoff `..._Slice2b_InlineCreate_..._v1.md`): the away-destination field links into the existing `/locales/new` flow with a `return_to` back to `/profile/event-windows` — reusing `new_locale`'s consent + search + `mapbox_id` dedup + the `_stash_return_to`/`_locale_flow_redirect` round-trip; template-only + 1 render test, no new route/schema/DDL (suite 2433/30).
