...
- **Re-pin: NOT NEEDED (auto-discovered).** Runtime resolves the active Layer-0 version dynamically — `_q_current_etl_version_set` (orchestrator.py:757) returns `MAX(etl_version) FROM layer0.sports WHERE superseded_at IS NULL`. Once the v11 re-extract supersedes the v10 `layer0.sports` rows, the runtime picks up `0A-v11.0` automatically. `race_events.etl_version_set` is row provenance (`race_events_v1` migration marker), NOT the Layer-0 pin — leave it alone. (The earlier "re-pin etl_version_set" wording assumed a deployed pin that doesn't exist; corrected here.) Confirm with `SELECT DISTINCT etl_version FROM layer0.sports WHERE superseded_at IS NULL` → expect only `0A-v11.0`.
...
- The data layer must land together with the code merge — runtime reads `layer0.*` by the auto-discovered active version; merged code (new ids) against an un-re-extracted Neon (old active ids) breaks every discipline lookup.
