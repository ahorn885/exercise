# Layer 4 Surface-Specific Routing Within a Discipline — Design Spec v2

**Status:** APPROVED-approach (Andy via `AskUserQuestion`, 2026-06-16 — chose "deterministic intent infra" over a prompt-only instruction or closing). **Slice 1 + Slice 2 both BUILT** (Slice 1 PR #639, merged; Slice 2 this session — for Slice 2 Andy chose the "count-deterministic, LLM picks vert/technical" binding depth over a fully-deterministic surface split or a prompt-only no-op). Slice 3 (craft disciplines) remains the open follow-up.
**Date:** 2026-06-16 (v2: Slice 2 built)
...
### Slice 3 (still deferred) — craft disciplines
Bike/paddle keep the existing `resolve_craft_terrain_feasibility` cascade; surface-purpose routing + session-typing there is a follow-up (the craft tier composes craft ownership with terrain). #624 stays OPEN, narrowed to Slice 3.
...
- **Missing:** craft disciplines (Slice 3) are still deferred; the vert-vs-technical choice per quality session is intentionally left to the LLM (the ratified binding depth).
