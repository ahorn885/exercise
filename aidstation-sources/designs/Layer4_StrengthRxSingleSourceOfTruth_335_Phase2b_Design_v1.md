...
**Status:** SHIPPED (2026-06-16). Decisions LOCKED (Andy): **D5 = phase-aware %1RM**, **D7 = identity + load model in one arc (split into PRs)**, **D2 = backfill history to EX-ids**, D3 crosswalk per §4. Implementation complete across 6 PRs: #664 (this design), #665 (`layer0_progression.py` crosswalk), #666 (`current_rx.layer0_exercise_id` column + `rx_engine.current_rx_by_layer0_id`), #667 (16-name backfill), #670 (4 new layer0 exercises EX246–EX249 + backfill, applied to prod), #671 (the `rx_wire` EX-id surface fix + phase-aware %1RM load, D5b). **D5b realization:** Epley est-1RM + Epley-inverse at the phase's prescribed reps (self-consistent; no separate %1RM table), rounded to a gym increment. Slice C (retire `public.exercise_inventory` route-by-route) remains #430's job, out of #335 scope.
