...
**Status:** signed off + **Slices 1 & 2 implemented** (2026-06-04→05, pv=57 stall investigation — strength surface was the upstream cause of the `equipment_unavailable` blockers + synthesis thrash). **Slice 1** = §8 exercise-surface rendering + §9 system-prompt section in `layer4/per_phase.py` (synthesizer picks real resolved `exercise_id`s instead of inventing them). **`movement_patterns` threaded** through 2C (`layer0.exercises.movement_patterns` → 2C query → `ResolvedExercise.movement_patterns`), so the pool now ranks by movement pattern (§8 bias) and derives a **data-driven core/accessory** split (core = priority Critical/High + compound pattern; priority *is* the sport-relevance ranking). **Slice 2** = the §10 `strength_frequency_band` advisory validator (Base/Build 2, Peak/Taper 1, ±1, `warning`-only) — catches the bare/empty-strength regression. **Owed (deferred):** §3 3A-strength-state *modulation* of the dose (low→keep 2×, high→maintenance-lean — the base dose ships; the soft ± is not wired); `exercise_type` pool filter left fail-open (`exercises_resolved` is already the 0B exercise set; over-filtering risks the empty-surface bug this fixes); the Taper "1 early then 0 final ~7–10 days" week-position nuance is prompt-only (the ±1 band tolerates it).
