...
4. **Retire the extractors + xlsx.** ✅ **Gate cleared 2026-06-11** — `0001` (D-007 cleanup) + `0002` (de-orphan `supplement_vocabulary`) + `0003` (de-orphan `terrain_gap_rules`) have gone through the gate cleanly (3 ≥ the 2–3 bar). The §6.4 freeze is now **safe to take**: archive `etl/layer0/extractors/`, `run.py`, `emit_sql.py`, and the two remaining workbooks (`Sports_Framework_v14.xlsx`, `AR_Exercise_Database_v19.xlsx`) the way `Project_Backlog_vN.md` was frozen. The old-version test pins are already resolved (§7.2 — tests run against v14, the live source). **This is the remaining open item in the epic.**
...
- [ ] Phase 4 — freeze extractors + remaining workbooks (v14/v19). _Gate **cleared** (§6.4; `0001` + `0002` + `0003` through cleanly) — the freeze is now safe to take; this is the last open epic item._
