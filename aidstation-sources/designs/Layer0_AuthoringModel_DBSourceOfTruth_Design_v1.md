...
4. **Retire the extractors + xlsx.** ✅ **DONE — frozen 2026-06-11.** Gate cleared by `0001` (D-007 cleanup) + `0002` (de-orphan `supplement_vocabulary`) + `0003` (de-orphan `terrain_gap_rules`) (3 ≥ the 2–3 bar), then `etl/layer0/extractors/`, `run.py`, `emit_sql.py`, the two workbooks (`Sports_Framework_v14.xlsx`, `AR_Exercise_Database_v19.xlsx`) + their 6 parser/canon/versioning tests were moved to **`etl/_frozen_xlsx_authoring/`** (archive with a `README` marker; the way `Project_Backlog_vN.md` was frozen). Three main-suite tests that imported the extractors were resolved: the ETL-vs-route terrain cross-check (`test_onboarding_race_events`) narrowed to the live route set; the parser tests in `test_layer2c_prep` retired (kept `TestSchemaSubstrate`); the entire `_TERRAIN_STRUCTURED_ROWS` terrain-vocab audit (`test_bucket_c_terrain_vocab_audit`) frozen with the source it tested, its integrity coverage **rebuilt DB-side** as a new `terrain_types` `validate_layer0` check (`validation/terrain_types_check.py` — TRN-NNN format + active-set uniqueness; the 9th gate check). Nothing live imports the freeze set; the gate (`schema.sql` + genesis + migrations + `validate_layer0`) + the `export_xlsx` hedge are untouched. **Epic #488 is complete.**
   - **Spec-sourced orphan tables — DONE.** Two Layer 0 tables — `supplement_vocabulary` and `terrain_gap_rules` — were hand-authored from specs, not ETL-emitted, so they were never in the genesis snapshot and lived as one-shot `etl/sources/migrate_*.sql` the gate never saw. Folding them into the gate-covered model was itself a freeze prerequisite (you cannot retire the authoring loop while Layer 0 tables are managed by ad-hoc SQL): each came in via **one self-contained migration** (`CREATE TABLE IF NOT EXISTS` + seed) — **not** `schema.sql`. The `TestLayer0TableFamilyMap` drift guard requires every `etl_version` table in `schema.sql` to be in `_LAYER0_TABLE_FAMILY` (the `{0A,0B,0C}` cache digest); `supplement_vocabulary` is intentionally outside that cone (Layer 2E, own `supp_vocab.*` version — slice 3b) so it *must* stay out of `schema.sql`, while `terrain_gap_rules` *is* a 0C cone table already in the family (the guard wouldn't object) but stays out for consistency. `0002` did `supplement_vocabulary` (subsuming the D-21 contraindication retag); `0003` did `terrain_gap_rules` (subsuming `populate_terrain_gap_rules.sql` + the `partial`→4-band severity reclassification), reproducing the exact live `0C-v2.0-r2` state.
   - **Spec-sourced orphan tables — DONE.** Two Layer 0 tables — `supplement_vocabulary` and `terrain_gap_rules` — were hand-authored from specs, not ETL-emitted, so they were never in the genesis snapshot and lived as one-shot `etl/sources/migrate_*.sql` the gate never saw. Folding them into the gate-covered model was itself a freeze prerequisite (you cannot retire the authoring loop while Layer 0 tables are managed by ad-hoc SQL): each came in via **one self-contained migration** (`CREATE TABLE IF NOT EXISTS` + seed) — **not** `schema.sql`. The `TestLayer0TableFamilyMap` drift guard requires every `etl_version` table in `schema.sql` to be in `_LAYER0_TABLE_FAMILY` (the `{0A,0B,0C}` cache digest); `supplement_vocabulary` is intentionally outside that cone (Layer 2E, own `supp_vocab.*` version — slice 3b) so it *must* stay out of `schema.sql`, while `terrain_gap_rules` *is* a 0C cone table already in the family (the guard wouldn't object) but stays out for consistency. `0002` did `supplement_vocabulary` (subsuming the D-21 contraindication retag); `0003` did `terrain_gap_rules` (subsuming `populate_terrain_gap_rules.sql` + the `partial`→4-band severity reclassification), reproducing the exact live `0C-v2.0-r2` state.
...
- [x] `0002` — de-orphan `supplement_vocabulary` into the gate (self-contained `etl/migrations/layer0/0002_seed_supplement_vocabulary.sql`, NOT `schema.sql`; subsumes the D-21 retag) (2026-06-11).
- [x] `0003` — de-orphan `terrain_gap_rules` into the gate (self-contained `etl/migrations/layer0/0003_seed_terrain_gap_rules.sql`; subsumes `populate_terrain_gap_rules.sql` + the severity reclassification; the last spec-sourced orphan) (2026-06-11). **Clears the 2–3-migration freeze gate.**
- [x] Phase 4 — freeze extractors + remaining workbooks (v14/v19) → **`etl/_frozen_xlsx_authoring/`** (2026-06-11). Gate cleared by `0001`+`0002`+`0003`; extractors/`run.py`/`emit_sql.py`/workbooks + their ETL-only tests archived; 3 main-suite tests resolved (see §6.4). **Epic #488 complete.**
