# Layer 4 — Per-week volume periodization grid (Design v2)

**Status:** SIGNED OFF + IMPLEMENTED (v1 2026-06-05; **v2 3A coupling shipped
2026-06-20, #424**). Curve parameters confirmed by Andy (§4 resolutions below).
Shipped in `layer4/periodization.py` + `validator.py`
(`phase_week_volume_bands_hours`, `_rule_volume_band`) + `per_phase.py`
(`_format_session_grid` / `recovery_week` stamp). **Q4 (3A coupling) is now
RESOLVED + shipped — see §4.4 + §13.**
...
- **Q4** 3A coupling (`recent_trajectory` / `data_density` / ACWR) — **RESOLVED:
  shipped in v2 (#424).** Modulates ramp slope + deload depth per athlete while
  preserving the volume-neutral invariant. Magnitudes in §4.4 / §13.

**Date:** 2026-06-05 (v1) · 2026-06-20 (v2 3A coupling)
...
## 4. The curve — **DECISIONS (RESOLVED)**

Everything below was a *proposal* for sign-off; all five are now resolved and shipped.

### 4.1 Volume-neutral vs peak-anchored — **Q1 (resolved: volume-neutral)**
**Volume-neutral** (§3.2).

### 4.2 Deload cadence & depth — **Q2 (resolved)**
- **Cadence:** mode-dependent (`standard 4` / `compressed 3` / `extended 5`), continuous across phase boundaries on the **plan-global** week index; `custom` → no auto-deload.
- **Depth:** `_M_DELOAD = 0.55` (~45% cut). *(v1 proposed 0.65; signed off at 0.55.)*

### 4.3 Per-phase ramp shape — **Q3 (resolved)**
A single deterministic ramp (`_RAMP_STEP = 0.08`/wk) that resets at each deload → a 3:1 sawtooth, renormalized so the phase mean = 1.0. Taper rides a fixed Bosquet descent (0.75 / 0.60 / 0.40 into race week, NOT renormalized). *(Simplified from v1's per-phase start/end-fraction table to one slope constant; the v2 3A factor — §4.4 — now personalizes that slope.)*

### 4.4 3A-driven modulation — **Q4 (RESOLVED — shipped in v2, #424)**

`recent_trajectory` is specified to "drive volume ramp shape," and §8.2 prescribes conservative ramps for sparse data. **v1 shipped the deterministic athlete-agnostic curve; v2 (#424) adds the 3A coupling as a multiplicative modulation on the ramp slope + the deload depth, preserving the volume-neutral invariant.**

`periodization.ramp_modulation_for(layer3a_payload) → RampModulation(ramp_step_factor, deload_multiplier)`:

| Signal | Source field | Effect on `ramp_step_factor` |
|---|---|---|
| Trajectory | `recent_trajectory.medium_term.direction` | `building` ×1.15 (steeper); `fatigued`/`overreached`/`detrained` ×0.85 (gentler); else ×1.0 |
| Data density | `data_density.recent_workouts_count` | `<5` ×0.80, `<12` ×0.90 (sparse → conservative); else ×1.0 |
| ACWR | `recent_trajectory.acwr_status.combined.zone` | `non_functional_overreach` ×0.70, `functional_overreach` ×0.85 (cap when already loaded); else ×1.0 — **never accelerates on an ACWR signal alone** |

The three sub-factors multiply, then clamp to `[0.5, 1.5]`. The factor scales the
ramp **STEP (slope)**, not absolute levels, so the phase-mean renormalization
(§3.2) keeps Base/Build/Peak **volume-neutral regardless of the modulation** — a
capped ramp just flattens the phase; it never changes the total (that stays the
2A allocation's job).

**Deload depth (Andy scope pick, #424):** fatigue signals
(`fatigued`/`overreached`/`detrained` direction OR `non_functional_overreach`
zone) ALSO **deepen the deload** — `deload_multiplier = _M_DELOAD − 0.075 ×
(signals)`, floored at `0.40` (60% cut, the bottom of the cited 30–60% band).
Renormalization keeps the phase volume-neutral, so a deeper deload sharpens the
recovery dip without shrinking the phase total.

Keying the ACWR cap on the **3A zone** (rather than the raw ratio) means the
no-chronic-base sentinel (acute>0, chronic=0 → ratio 999) lands in
`non_functional_overreach` and is capped correctly. Absent payload →
`NEUTRAL_MODULATION` → the v1 athlete-agnostic curve (graceful degradation).
Magnitudes are v1 defaults (`_K_*` / `_DELOAD_DEEPEN_STEP`), tunable
post-launch per `Layer4_Spec.md` §8.3.
...
- **v2 (#424):** `phase_week_volume_bands_hours` gained an optional `layer3a_payload` arg (passed `ctx.layer3a_payload`) → the validator band bends per-athlete in lockstep with the prompt target.

*Net:* fewer special cases, not more. The grid replaces the demotion zoo.

## 6. Prompt changes (`layer4/per_phase.py`, `_format_session_grid`)
...
- **v2 (#424):** `_format_session_grid` / `build_session_grid` thread `layer3a_payload` (from `render_user_prompt`) so the rendered per-week targets carry the same per-athlete modulation the validator grades against.
...
- *Unaffected by v2:* the deload **cadence** (which weeks) is unchanged; v2 only changes the deload **depth** (the band value), so the stamper needs no payload.
...
- **Unit (v2 3A modulation, #424):** per-signal factor mapping; product clamp; **volume-neutrality preserved under any modulation**; threading through `phase_week_multipliers_for_phase`; `None` → neutral; defensive log.
...
- **v2 risk:** the modulation could over-steepen / over-flatten a curve. Mitigated by the `[0.5, 1.5]` clamp, the preserved volume-neutral invariant (totals unchanged regardless of factor), and `None`-payload graceful degradation to the v1 curve.
...
4. **v2 (#424):** 3A coupling threaded into the existing single-chokepoint grid — no new call sites.

---

## 12. Open questions (consolidated — RESOLVED)

- **Q1** Volume-neutral (proposed) vs peak-anchored band? (§4.1) — **volume-neutral.**
- **Q2** Deload cadence (every 4th plan-global week?) and depth (`m_deload`?). (§4.2) — **mode-dependent cadence; depth 0.55.**
- **Q3** Per-phase ramp start/end fractions. (§4.3) — **single `_RAMP_STEP = 0.08` slope + Bosquet taper.**
- **Q4** Defer 3A `recent_trajectory` / ACWR / `data_density` coupling to v2? (§4.4) — **shipped in v2 (#424).**
- **Q5** Land WS-1 (forced-tool) now or bundle with the grid PR? — landed separately.

---

## 13. v2 implementation note (#424, 2026-06-20)

**Files:** `layer4/periodization.py` (`ramp_modulation_for`, `RampModulation`,
`log_ramp_modulation`, `_K_*` / `_DELOAD_*` constants; `phase_week_multipliers`
gained a `modulation` arg; `phase_week_multipliers_for_phase` /
`week_volume_multiplier` gained an optional `layer3a_payload`),
`layer4/validator.py` (`phase_week_volume_bands_hours` + its call site pass
`ctx.layer3a_payload`), `layer4/session_grid.py` (`build_session_grid` arg),
`layer4/per_phase.py` (`_format_session_grid` threads `render_user_prompt`'s
`layer3a_payload` + one-per-phase Rule-#15 log), `tests/test_layer4_periodization.py`
(+10 cases).

**Single chokepoint:** both the validator band and the prompt target route
through `phase_week_multipliers_for_phase`, so the payload only had to reach
that one function for band + prompt to stay consistent.

**No schema / inter-layer contract / cache-revision change** (deterministic grid
math; prompt *wording* unchanged → no `LAYER4_PROMPT_REVISION` bump). Defaults
preserve the v1 curve for any payload-less caller.
