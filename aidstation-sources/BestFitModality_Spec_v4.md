...
**Status:** Spec. Slice 1 (remove race-craft-aware scoring) + Slice 2 (vocab / pure-craft display-name overlay) shipped. **Slice 3 (data model) is satisfied on-disk under the ratified flat-list encoding — see §0 R5 + §12.** The next forward move is Slice 4.
...
- **Slice 4 — Layer 2B per-discipline terrain gaps (← next forward move):** key the terrain-gap output by discipline. The `RaceTerrainEntry.discipline_id` field already exists *and is now captured* by the form (Slice 3 / Bucket E) — Slice 4 is the first slice to *consume* it. Trigger #3 (Layer 2B contract change). Decide here whether the discipline tag should become required per row (see Slice 3 note).
