...
**Status:** Spec. Slice 1 (remove race-craft-aware scoring) + Slice 2 (vocab / pure-craft display-name overlay) + **Slice 4 (Layer 2B per-discipline terrain gaps, shipped 2026-05-25)** shipped. **Slice 3 (data model) is satisfied on-disk under the ratified flat-list encoding — see §0 R5 + §12.** The next forward move is **Slice 5** (resolver re-model + Layer 4 craft reasoning).
...
- **Slice 5 — resolver re-model + Layer 4 craft reasoning (← NEXT FORWARD MOVE):** implement §5; the per-discipline resolver consumes `terrain_by_discipline` (§5.1 ranks each leg's terrain emphasis by `pct × fidelity`); assemble the craft candidate set (§5.2); thread it + terrain emphasis into the Layer 4 prompt; update the cache key (§9, replacing v2's `race_modality_hints_hash` slot). Trigger #1 (prompt change).
