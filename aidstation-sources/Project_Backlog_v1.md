...
| D-07 | `phase_load_weekly_totals` — 16 rows short. **Verified 2026-05-10:** 4 missing sports are Off-Road / Adventure Multisport (Non-Nav), Open Water Marathon Swimming (10km / Olympic Distance), Open Water Marathon Swimming (25km / Ultra Distance), Swimrun. AR has all 4 phase rows ✓ | Med | 🟡 Deferred (AR-safe) — **investigation query queued FC-1a** | 2E, 4 | All 4 missing sports have aggregator rows in `phase_load_allocation` but didn't parse to weekly_totals. Likely a parser bug specific to those sports' WEEKLY TOTAL TARGET row formatting. Investigation queries in `FC1a_Pass1_Bundle.md` §7. Once 4 names confirmed via query, next step is xlsx inspection of those sports' WEEKLY TOTAL TARGET row formats. |
| D-08 | `sport_discipline_map` — 3 rows missing in deployed (73 source vs 70 deployed). **Verified 2026-05-10:** 3 missing rows are Long Distance / Endurance Cycling (-2: 5 in xlsx, 3 deployed) and Triathlon (-1: 5 in xlsx, 4 deployed). AR has all 15 disciplines ✓ | Med | 🟡 Deferred (AR-safe) — **investigation query queued FC-1a** | 2A | Specific missing discipline IDs not yet identified — `Long Distance / Endurance Cycling` is at top-level Sheet 3 naming so could be any of the 5 sub-disciplines that are absent. Investigation queries in `FC1a_Pass1_Bundle.md` §8. |
...
| D-22 | **`exercises.injury_flags_text` → structured `movement_components TEXT[]` promotion.** Free text in this column is the only source of "movement constraint × exercise" overlap for 2D. Keyword matching is heuristic with known false-negative risk. Promotion to structured TEXT[] enables set-intersect against B.3 enum tokens (mathematically exact). **Population: 159 active rows** (per Neon `layer0.exercises` query 2026-05-11; prior "~600 cells" estimate was wrong). Cross-layer note exists in `Athlete_Onboarding_Data_Spec_v2.md` §B.3. | High | 🔴 **Blocker for 2D implementation (FC-1b)** | 2D | **Pass 1 sampling complete 2026-05-11:** 57 rows classified (36% of population), house rules locked, edits applied per Andy review. Remaining 102 rows + migration SQL pending FC-1b. House rules + Pass 1 baseline live in `D22_Curation_Reference.md`. Spec'd in Layer2D_Spec §5.5 / §6. |
...
| D-25 | **§I v3 polish candidates** — 10 onboarding refinement items parked during §I structured rewrite. | Low | 🟢 Cleanup | Layer 1 v3 | See `Section_I_Audit.md`. v3 onboarding pass, not now. Not blocking 2E. |
| D-26 | **`supplement_vocabulary` Layer 0 table implemented in FC-1.** | High | ✅ Resolved (FC-1, 2026-05-11) | 2E | Table deployed to Neon dev with 25 seed entries per `Supplement_Vocabulary_Spec.md`. Schema: 8-value `category` enum + 4-value `evidence_quality` enum + structured `contraindications TEXT[]` for 2E coaching-flag cross-ref against §B. 2E supplement FK reads now valid. ETL tag: `supp_vocab.v1.FC1`. Migration: `etl/sources/migrate_supplement_vocabulary.sql` (already in `main` from IJKL-drift commit 50785df; deployed this session). |
| D-27 | **Plan Management spec not yet written.** 2E names contracts for `HeatAcclimState`, `expected_race_temp_c`, and `current_phase` source-of-truth that Plan Management must honor. Plan Management subsystem also handles weight staleness advisories and adherence-drop logic (see `Adherence_Drop_Spec_v2.md`). | High | 🟡 Deferred | 2E, future Layer 3/4 | Spec lands post-Layer-3 design. 2E ships with documented contract; Plan Management implementation comes later. Named contract is the protection. |
...
Every layer/sublayer designed from this point on gets its own `LayerNX_Spec.md` at the `Layer2C_Spec.md` depth standard. Handoff docs are session bookkeeping; specs are source of truth. New spec → update `Control_Spec.md` §9 doc map.
...
## Final cleanup batch — provisional scope

Two sessions at end of Layer 2 design. Split:

### Session FC-1: ETL bug fixes + column promotions

**ETL bug fixes (FC-1a status, 2026-05-11):**
- ~~D-04~~ — ✅ Resolved. Rename migration confirmed deployed state matches target.
- ~~D-05 (cleanup half)~~ — ✅ Cleanup SQL ran (no-op; 33 rows already superseded). **ETL extractor code patch still pending — CC.** Standing rule stays until next clean ETL run.
- ~~D-06~~ — ✅ Resolved. Columns renamed `weekly_low_hours → hours_low`, `weekly_high_hours → hours_high`.
- ~~D-12~~ — ✅ Resolved. Schema block drafted in `Layer0_ETL_Spec_v3_Patch_Batch_D.md`.
- ~~D-13~~ — ✅ Resolved. Correction drafted in `Layer0_ETL_Spec_v3_Patch_Batch_B_Correction.md`.
- ~~D-16~~ — ✅ Resolved. Type confirmed `TEXT[]`; transform documented in Batch B Correction.
- D-03 — Decision locked: **BUILD** `is_conditional` + `vertical_gain_notes` in v4 ETL (Andy confirmed 2026-05-11). Implementation deferred to v20 ETL re-run.
- D-07 — fix `phase_load_weekly_totals` parser for the 4 missing sports: Off-Road / Adventure Multisport (Non-Nav), Open Water Marathon Swimming (10km), Open Water Marathon Swimming (25km), Swimrun. **Investigation query queued in `FC1a_Pass1_Bundle.md` §7.**
- D-08 — investigate `sport_discipline_map` 3 missing rows. **Investigation query queued in §8.**
- D-14 — `cross_sport_properties.source_text` dedup. **Investigation query queued in §9; decision pre-approved.**
- D-15 — `discipline_substitutes` UNIQUE tighten. **Investigation query queued in §10; decision pre-approved.**

**Column / table promotions (Blocker class — 2D and 2E implementation depends on these):**
- D-22 — promote `exercises.movement_components TEXT[]` (Pass 1 sampling done 2026-05-11; 57/159 classified, house rules locked, ~102 rows + migration SQL remaining for FC-1b. **Population: 159, not 600 as originally estimated.**)
- D-23 — promote `disciplines.body_parts_at_risk TEXT[]` (~150 cells curation; seed drafted in Layer2D_Spec §5.5)
- ~~D-26~~ — ✅ Resolved 2026-05-11. `supplement_vocabulary` table deployed to Neon dev. Migration in `etl/sources/`.

**Capacity contingency — FC-1a / FC-1b split: CONFIRMED 2026-05-11.** D-22 Pass 1 sampling showed ~1.5–3 min/row review pace, projecting ~2–4 hours for remaining 102 rows. Combined with D-23 + ETL fixes, single FC-1 session is too much.
- **FC-1a (in progress 2026-05-11)** — ETL bug fixes (D-04, D-05, D-06, D-07, D-08, D-12, D-13, D-14, D-15, D-16) + D-03 build-or-drop decision. **6 of 11 resolved; 4 investigation queries queued; D-03 decided.**
- **FC-1b** — D-22 batch 2 (remaining 102 rows of curation) + D-23 (body_parts_at_risk curation) + migration SQL for both

Pre-curator question — **D-26 `evidence_quality` rating source: resolved by deployment.** Table shipped 2026-05-11 with the per-row evidence ratings as drafted in `Supplement_Vocabulary_Spec.md` §2. Re-audit if athlete cohort data later contradicts any single rating; switching to a single canonical source (AIS / Examine.com / other) becomes a v2 vocab refresh, not a v1 blocker.

**Note:** D-17 (sport naming convention mismatch) was removed from FC-1 — it's a Layer 1/2A design requirement, not an ETL cleanup. Resolution belongs in the Layer 1 race-goal capture spec.

### Session FC-2: Spec v4 rewrite
- Fold Batches A, B, C, D, B-Correction, this report's drift items into a unified `Layer0_ETL_Spec_v4.md`
...
- Apply all §8 consumer table corrections from `ETL_Spec_v3_Corrections_2ABC_v2.md`
