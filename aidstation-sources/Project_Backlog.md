...
| D-03 | `phase_load_allocation` — `is_conditional BOOLEAN` and `vertical_gain_notes TEXT` defined in spec, NOT in deployed | High (spec-ahead) | 🟡 Deferred | 2A (discipline classification), 2D (injury risk), 4 (plan-gen) | All data exists in `notes_conditions` raw text; any consumer can derive. Decide at final cleanup whether to build the ETL derivations or drop from spec. |
| D-04 | `phase_load_allocation` — column-name `phase_` prefix in spec, no prefix in deployed | Med | 🟢 Cleanup | All | Pure rename. |
...
| D-06 | `phase_load_weekly_totals` — `hours_low/high` renamed to `weekly_low_hours/high_hours` | Low | 🟢 Cleanup | 2E (nutrition uses hours), 4 (plan-gen) | Pure rename in spec. |
| D-07 | `phase_load_weekly_totals` — 16 rows short. **Verified 2026-05-10:** 4 missing sports are Off-Road / Adventure Multisport (Non-Nav), Open Water Marathon Swimming (10km / Olympic Distance), Open Water Marathon Swimming (25km / Ultra Distance), Swimrun. AR has all 4 phase rows ✓ | Med | 🟡 Deferred (AR-safe) | 2E, 4 | All 4 missing sports have aggregator rows in `phase_load_allocation` but didn't parse to weekly_totals. Likely a parser bug specific to those sports' WEEKLY TOTAL TARGET row formatting. FC-1 fix. |
| D-08 | `sport_discipline_map` — 3 rows missing in deployed (73 source vs 70 deployed). **Verified 2026-05-10:** 3 missing rows are Long Distance / Endurance Cycling (-2: 5 in xlsx, 3 deployed) and Triathlon (-1: 5 in xlsx, 4 deployed). AR has all 15 disciplines ✓ | Med | 🟡 Deferred (AR-safe) | 2A | Specific missing discipline IDs not yet identified — `Long Distance / Endurance Cycling` is at top-level Sheet 3 naming so could be any of the 5 sub-disciplines that are absent. Investigation owed in FC-1. |
...
| D-12 | `sport_name_aliases` — schema not documented in §4 (only mentioned in §6.2 run order) | Med (doc) | 🟢 Cleanup | 2C, 2A (via bridge) | One-paragraph addition to v4. |
| D-13 | `discipline_technique_foci` — Batch B patch says `source_exercise_id` singular; deployed is `source_exercise_ids[]` array. Patch also missing `audit_log` col | Low | 🟢 Cleanup | 4 (plan-gen consumes foci) | Patch correction before it merges to v4. |
| D-14 | `cross_sport_properties` §4.8 — deployed has 4 extra cols (`source_evidence`, `notes`, `source_text`, `confidence`). `source_text` is suspicious — duplicates `source_evidence` semantically | Low | 🟢 Cleanup | None currently | Decide on `source_text` dedup during final cleanup. |
| D-15 | `discipline_substitutes` §4.9 — UNIQUE in deployed includes `substitute_name`; spec doesn't. Loose constraint allows duplicate (target, substitute) with different names | Low | 🟡 Deferred | 2D? (substitution logic), 4 (plan-gen) | Data check needed before tightening. Probably no current conflicts. |
| D-16 | `primary_muscles` / `secondary_muscles` — Open Item R now resolved; spec v3 §4.12 doesn't list them | Low | 🟢 Cleanup | None (data is fine; doc gap) | Added to Batch B/C patches. v4 schema block must include them. |
...
- D-07 — fix `phase_load_weekly_totals` parser for the 4 missing sports: Off-Road / Adventure Multisport (Non-Nav), Open Water Marathon Swimming (10km), Open Water Marathon Swimming (25km), Swimrun
- D-08 — investigate `sport_discipline_map` 3 missing rows in Long Distance / Endurance Cycling (2) and Triathlon (1); identify specific discipline IDs absent
- D-03 — decide build-or-drop on `is_conditional` and `vertical_gain_notes`
- D-13 — correct Batch B patch (`source_exercise_ids[]`, add `audit_log`)
- D-14 — resolve `cross_sport_properties.source_text` dedup
- D-15 — data check on `discipline_substitutes` UNIQUE; tighten if no conflicts
...
- **FC-1a (next session)** — ETL bug fixes (D-04, D-05, D-06, D-07, D-08, D-12, D-13, D-14, D-15, D-16) + D-03 build-or-drop decision
...
- Fold Batches A, B, C, this report's drift items into a unified `Layer0_ETL_Spec_v4.md`
- Add missing §4 sections for `terrain_gap_rules`, `sport_name_aliases`, `discipline_technique_foci`
