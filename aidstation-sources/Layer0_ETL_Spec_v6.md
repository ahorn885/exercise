# Layer 0 ETL Specification — v6

**Version:** 6.0 (file revision); schema version v3 (unchanged — v6 closes the FC-3 terrain_types deferral and surfaces the FC-3 D-46 follow-up audit outcome)
**Status:** Revised 2026-05-13 (FC-4a). v6 closes D-41 (`terrain_types` 9-column schema enumerated; drift report's `simulatable BOOLEAN` claim corrected to `TEXT`) and D-46 (`sport_name_aliases` multi-mapping audit complete — 21 multi-mapped rows confirmed intentional for framework sub-format splitting; consumer-side dedup requirement documented in §4.11). New cleanup item D-47 surfaces a Layer 2D §5.2 rationale-comment gap (functional behavior already correct via dedup-by-exercise_id).
**Supersedes:** `Layer0_ETL_Spec_v5.md` (FC-3, 2026-05-13).
**Sources:**
...
- Project Backlog (`Project_Backlog_v8`) is the active cross-layer tracker; spec catches up to backlog decisions as they land.
...
**Active tracker:** `Project_Backlog_v8.md` is the single source of truth for cross-layer drift items, blockers, deferred work, and cleanup. This section preserves the v3 enumeration for historical continuity but does not maintain a parallel live list. Refer to the backlog for current state.
...
**v4 reconciliation summary — items now in `Project_Backlog_v8.md`:**
