...
> **#334 amendment 2026-05-31 (terminal-phase floor — event mode must reach race day).** In **event mode**, the last phase in the allocated structure (the phase whose `end_date` is the plan's `scope_end_date`) is guaranteed **≥ 1 week**. The pre-amendment rounding (`int()` truncation in `phase_structure.py::_allocate_weeks_standard`) could allocate the terminal phase 0 weeks when its proportional share was < 1 week (e.g. a 6-week `standard` plan from `start_phase='Build'` gave Build 5 / Peak 1 / **Taper 0** — the 0.6-week Taper floored away), so the structure ended on Peak and the plan stopped a full week short of the event. Rule: when `total_weeks` admits ≥ 2 phases for the mode/`start_phase`, the terminal phase is rounded **up** to a minimum of 1 week, and the extra week is reclaimed from the largest non-terminal phase (Base first when present, else the largest remaining) so `sum(weeks) == total_weeks` is preserved. This makes the §6.1-budgeted Taper actually survive to drive the Decision-4 race-week machinery (auto-emitted Taper `coaching_flags` + `llm_layer4_race_week_brief`, which only modifies existing Taper-phase sessions and produces none of its own — so a dropped Taper silently breaks the race-week brief too). Open-ended mode is unchanged (no event, no race-day boundary to honor). Custom mode (`phase_weeks` verbatim) is unchanged. When `total_weeks` is so small only one phase fits, that single phase stands as the terminal phase (no synthetic second phase is introduced).
