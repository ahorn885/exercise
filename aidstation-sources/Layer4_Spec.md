...
**Known limitation — per-block synthesis cost (2026-05-27, OPEN).** D-77 Slice 1 shrank the *cache unit* (phase → week) but did NOT bound a single block's *synthesis cost*: `per_phase.synthesize_phase` runs a capped-retry loop (up to `capped_retries+1` extended-thinking calls, and each `caller()` may add a forced-tool fallback per the #174 thinking-miss path) that is **in-memory, not resumable across function invocations**, and the block caches only after the loop returns. So a block that needs retries (schema-miss or per-block validator rejection) can run past the immovable 300s Vercel cap, 504-kill mid-loop, cache nothing, and be restarted from attempt 1 on the next pass — an infinite 504 loop, the SAME failure mode as the original `plan_version_id=23` incident, now caught loudly by the wall-clock backstop above instead of looping forever (observed: `plan_version_id=25`, warm 3A/3B cone, block synthesis reached immediately, every pass 504 with an Anthropic call in flight, zero blocks cached in ~900s). Diagnostic instrumentation is in place (`synthesize_phase` logs per-attempt latency + the previously-silent validator-rejection rule set under a greppable `<phase>:w<week>` tag) so a re-run reveals whether retries are a fixable validator/synthesis bug or a genuine cost overrun. The bounding fix (candidates: a per-block wall-clock budget that accepts best-effort once spent; lower retry/thinking budget for blocks; finer-than-week decomposition) is deferred pending that read — Trigger #5.
