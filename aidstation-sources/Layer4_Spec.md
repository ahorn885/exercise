...
- **D-66** race-event data model — **✅ Design wave shipped 2026-05-18** per `Race_Events_D66_Design_v1.md`. Schema (`race_events` + `race_route_locales` + `race_route_locale_equipment` tables) + `RaceEventPayload` typed model + Layer 4 §3.4 amendment (new `race_event_payload` arg) + onboarding §A + §K design + profile UI design landed in the design wave. Implementation (DDL migrations + RaceEventPayload pydantic model + onboarding/profile UI code) is in subsequent PRs. Rule `kit_manifest_inputs_incomplete` is now non-trivially evaluable per §5.4 row above (D-66 active branch).
