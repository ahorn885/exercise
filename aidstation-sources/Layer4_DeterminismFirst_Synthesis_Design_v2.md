# Layer 4 Determinism-First Synthesis — Design Spec v2

**Status:** APPROVED (Andy, 2026-06-06). Slice 2a + 2b + 2c + 2d shipped — Track 2 closed. Slice 2c cardio-routing pulled out → 2c.2 follow-up (depends on layer0 vocab work). **v2 (2026-06-06):** adds the session-count ceiling fix (D11 / §5.1.1 / slice 2b.2) after cold PGE plan #60 failed on an unschedulable 14-session Build week.
**Date:** 2026-06-06 (v2)
...
### 5.1.1 Session-count ceiling + athlete two-a-day preference (D11, v2)

**Problem (cold PGE plan #60, 2026-06-06).** §5.1 computes per-discipline counts bottom-up and **sums them with no global ceiling and no awareness of available training days**. For a 7-discipline athlete the ≥1-per-discipline floor alone forces ~7, and the higher-volume disciplines push the total to 14. With 6 available days (1 rest day) the hard schedulable maximum is `2 × 6 = 12`, so 14 is **mathematically unpackable** under the `two_per_day` invariant (`payload.py:_check_two_per_day`). The synthesizer then either spreads onto the rest day (`schedule_violation` — now a warning, so it silently burns the rest day) or clusters 3 sessions on one day (`two_per_day` → ValidationError at payload construction → **every** attempt fails → the block never caches → stall backstop → plan fails). Plan #60 died this way on Build:w2.

**Fix.** Cap the weekly session total at a deterministic, athlete-controlled ceiling derived from available days, anchored on Peak and scaled per phase. No LLM — pure arithmetic in `session_grid.py`.

**Athlete inputs (Layer 1 §K):**
...
**Applying the ceiling (deterministic shed, lowest-priority first).** After §5.1 computes raw per-discipline `sessions_this_week`, let `total = Σ sessions`. While `total > phase_ceiling`:

1. **Trim a multi-session discipline first** — among allocations with `sessions_this_week > 1`, pick the lowest `load_weight` and decrement it. This preserves discipline breadth: every discipline keeps ≥1 before any is dropped.
2. **Only when all are at 1** — drop the lowest-`load_weight` discipline to 0 for this week and set `cadence_note = "deferred — weekly capacity ceiling; rotates in on a lighter week"` (same maintenance-rotation spirit as §5.1 step 6).

Net effect: the dominant disciplines keep their frequency; the least-important volume sheds first.
...
| File | Change |
|---|---|
| `layer4/session_grid.py` | NEW `apply_session_ceiling(allocations, phase_name, available_days, two_a_day_preference, peak_sessions_max)` (§5.1.1) — density map, Peak anchor, per-phase scale, `2×D` clamp, lowest-`load_weight` shed (trim multi-session before dropping a discipline). `build_session_grid` gains `available_days` + `two_a_day_preference` + `peak_sessions_max` params and applies the ceiling after per-discipline allocation. |
| `layer4/per_phase.py` | At the `build_session_grid` call site (`per_phase.py:802`), pass `available_days` + the two prefs read from the Layer 1 payload (`available_days_per_week` / `daily_availability_windows` already in scope for the prompt's Schedule section). |
| `init_db.py` | NEW `athlete_profile` columns `two_a_day_preference TEXT` + `peak_sessions_max INT` via `_PG_MIGRATIONS` (nullable, defaulted, idempotent). **Migration runs on Andy's hands** — Neon egress is blocked from the container. |
| `layer4/context.py` | Add `two_a_day_preference` + `peak_sessions_max` to the Layer 1 §K schedule schema + payload. |
| `routes/profile.py` + onboarding §K template | New schedule question ("How often do you want to train twice a day?" → never/occasionally/regularly) + advanced "Most sessions per week at Peak" number; reject `peak_sessions_max > 2 × available_days_per_week`. |
