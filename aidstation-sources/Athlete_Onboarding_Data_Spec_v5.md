# Athlete Onboarding Data Spec — v5

**Version:** 5.0
**Status:** Onboarding Design Wave consolidated. D-58 + D-59 + D-60 + D-61 absorbed into the spec body. Spec body complete pending Open Items.
**Last updated:** 2026-05-14
**Supersedes:** `Athlete_Onboarding_Data_Spec_v4.md` (v4.0, 2026-05-13)
**Built:** May 2026
...
| Field | Type | Tier | Drives | Source |
|---|---|---|---|---|
| Daily Windows | Repeating per day of week (Sun–Sat): `enabled` (Boolean) + `window_start` (time-of-day) + `window_duration` (minutes, integer 30–360) + optional `second_window_start` + `second_window_duration` (gated on Doubles Feasible ≠ No) | 1 | Hard scheduling envelope per day; plan-gen places sessions inside windows; days with `enabled=FALSE` are unavailable for any non-rest session | Self-report |
...
| Preferred Rest Day(s) | Day-of-week single or multi-select | 2 | Soft rest-day preference signal for plan-gen ("I'd rather rest Sunday than Saturday"). **Demoted Tier 1 → 2 in v5** — most rest-day preferences are already captured by the `enabled` flag on each day | Self-report |

### G.2 Derived / displayed values (not entered)

The following v4 fields are dropped as explicit inputs and surfaced as derived values in the §G UI:
...
- Long-Session-day must be a subset of `enabled=TRUE` days. UI cross-validates: deselecting Saturday from `enabled` invalidates a Long-Session-day-on-Saturday picker.
...
| Dietary Pattern | Multi-select | 2 | Nutrition recommendations | Self-report |
...
Unchanged from v4. Fueling Format Preference, Known Race-day GI Triggers, Salt / Electrolyte Tolerance.
...
| **Athlete updates `daily_availability_windows` for a day** | Re-validate any in-flight plan sessions for that day; flag sessions that no longer fit |
| **Athlete updates Long Session Available** | Re-validate long-session placement; flag if the new day picker excludes the currently-scheduled long-session day |
