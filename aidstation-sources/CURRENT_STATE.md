...
`handoffs/V5_Implementation_FormRefresh_A2_DropAidStations_RouteLocalesAllTypes_Portage_2026_05_25_Closing_Handoff_v1.md` — 2026-05-25 (on branch `claude/form-refresh-unblocked-ywib4`; not yet merged)

FormRefresh A2 — slice-A remainder (item 3), re-scoped by Andy at the gate. **Deleted the `aid_stations` count column end-to-end** (form inputs on both surfaces, `RaceEventPayload`/`Layer2ETargetEvent` fields, orchestrator threading, repo CRUD, migration `DROP COLUMN IF EXISTS`) — race-day aid logistics live on the `race_route_locales` graph (`role='aid_station'`), and the count's only consumer was **Layer 2E HITL gate 5 (anaphylaxis × aid), which was removed** (Andy never wanted that scenario; `_emit_hitl_items` kept as a no-op for the deferred gates 1–4). **Route-locale capture now offered on every event type, always optional** (removed the onboarding `single_day` gating + the discouraging profile copy). **Portage** captured via **helper text** → Notes field (no schema; the `mandatory-gear → structured pack-weight` half was dropped — not wanted). 10 substantive files + 5 test suites; container suite 1641 passed/16 skipped (fresh-venv measurement — see handoff §1). **NEW owed Neon deploy (Andy's hands):** `python init_db.py` now also drops `aid_stations` (idempotent; same command as the A1 race_events migration). Full record + decisions in the handoff above.
...
- **Owed Neon deploys (Andy's hands — no `DATABASE_URL` in build container).** Two channels: (1) public schema — `python init_db.py` applies the FormRefresh A1 `race_events` migration **+ the A2 `aid_stations` `DROP COLUMN`** (one idempotent run); (2) layer0 — `psql "$DATABASE_URL" -f etl/sources/run_owed_layer0_migrations.sql` batches PR #156 `primary_movement` (HARD prereq — Layer 2A reads it), K3 equipment, + the D-74 idempotency re-runs in order.
