# Athlete Onboarding Data Spec — v4

**Version:** 4.0
**Status:** Batches 1–5 complete + §H.3 Non-Event Goal Type (v3) + scope trims (v4). Spec body complete pending Open Items.
**Last updated:** 2026-05-13 (v4.0: pregnancy intentionally not captured; shooting/fencing technical readiness removed per L3-Spec-Trio feedback)
**Supersedes:** `Athlete_Onboarding_Data_Spec_v3.md` (v3.0, 2026-05-13)
**Built:** May 2026

## What changed in v4 vs v3

Two scope clarifications per L3-Spec-Trio feedback:

1. **Pregnancy intentionally never captured.** v3 left a placeholder noting "a future profile field for pregnancy state could suppress the trigger automatically." Reversed. AIDSTATION does not capture pregnancy status as a profile field — ever. Female athletes see a static disclosure ("If you are pregnant, read this") at appropriate UI surfaces; rationale is privacy + medical-deference (pregnancy training guidance is OB/midwife territory, not coaching app). §B.4.2 "Out of scope for launch" updated; the post-launch design item is removed.

2. **No shooting or fencing technical readiness — ever.** v1–v3 §D.7 included Fencing Experience (épée) and Shooting Experience fields, and §J.3 included Fencing setup and Shooting setup gear toggles. All four removed. AIDSTATION does not coach or program shooting or fencing technique. Modern pentathletes and biathletes are still welcome — AIDSTATION programs their endurance, strength, and recovery; skill-discipline training comes from external resources (fencing coach, range time, governing-body programs). §D.7 retains only Rock Climbing and Abseiling.

No vocabulary changes elsewhere. No tier changes elsewhere. No changes to §H goal-type structure. No changes to Layer 3 input contracts.

**Deferred to dedicated design sessions (new Project_Backlog rows D-58 through D-61):**
- D-58: account/integration connections precede onboarding data entry (OAuth-first flow + integration-data prefill)
...
These four architectural items materially restructure §A (flow), §B (locales), §J (equipment/sessions). Out of scope for v4 surgical pass; flagged so that future spec versions don't re-litigate fields that will move.

---

## What changed in v3 vs v2

Single-purpose amendment: closes the goal-type gap in §H.3 no-event mode.

Three of the AIDSTATION-supported goal types ("endurance capacity," "overall fitness," "strength and muscular development") had no field in §H.3 — only Plan Duration was captured. Layer 3 node 3B (goal-timeline-viability evaluation, per L3-Discovery 4-node reshape) needs a goal-type signal in no-event mode to frame viability differently from event-mode (where §H.2 Goal Outcome supplies the signal).

**Added to §H.3:**
- `Non-Event Goal Type` — single-select enum (Endurance / General fitness / Strength / Mixed), TIER 1, default = General fitness, backwards-compatible

**Added narrative paragraph in §H.3:** explains the field's relationship to §C Discipline Weighting (allocation across disciplines) vs Non-Event Goal Type (type of progression targeted).

No other sections changed. No vocabulary changes. No tier changes elsewhere.

---

**Predecessor history:**
- v2.5 (2026-05-06) — §B.1.1 Bone split into non-stress / stress fracture
- v2.4 — §B.1.1 Joint mechanical split into surgical / non-surgical
- v2.3 — batch 4+5 integration + §B.1.2/§B.4.2 patches + Rule 3 sex-adjustment
- v2.0–v2.2 — Batches 1–5 drafted

**Sources reconciled (carried forward from v2):**
- v1 spec
...
**New for v3:**
- `L3_Discovery_Closing_Handoff_v1.md` §2.2 + §6.1 (amendment specification)
...
- Pregnancy-state RHR adjustments. Pregnancy elevates resting heart rate by 10–20 bpm during the second and third trimesters; pregnant athletes may see Rule 2 (elevated RHR) false-positive suggestions. AIDSTATION **does not capture pregnancy status** as a profile field — by design. Pregnancy-specific training adaptation is the domain of an OB, midwife, or qualified pre/postnatal coach, not a general-population endurance coaching app. UI surfaces a static disclosure to female athletes ("If you are pregnant or postpartum, training guidance from AIDSTATION should be reviewed with your healthcare provider"); athletes can dismiss false-positive RHR suggestions with no system-side memory penalty. No profile field, no inferred state, no post-launch design item.
...
The v1 §E.1 "Pack Load Training History" row is removed (moved to §C). §D.7 is renamed **Technical Disciplines** (was "Strength & Technical Disciplines"); strength benchmarks Pull-Up Maximum, Dead Hang Duration, Grip Strength move to §E. §D.7 retains Rock Climbing and Abseiling only. (v3 listed Fencing and Shooting here; v4 removed both — AIDSTATION does not program shooting or fencing technique. See "What changed in v4 vs v3.")
...
(Renamed from v1 §E.7 "Strength & Technical Disciplines." Strength benchmarks — Pull-Up Maximum, Dead Hang Duration, Grip Strength — moved to §E. v4 removed Fencing and Shooting Experience fields — see scope note below.)
...
**Scope note — shooting and fencing.** AIDSTATION does not program shooting or fencing technique. Modern pentathletes and biathletes are supported athletes — AIDSTATION covers their endurance, strength, recovery, and general conditioning — but skill-discipline training (range time, fencing footwork and bouts, equipment-specific drills) is the domain of a discipline-specific coach or governing-body program. The athlete is responsible for their own technical training in these areas; AIDSTATION makes no representation about technique readiness for them. No fields collected; no plan-side programming generated.
...
§D.7 Technical Disciplines (Rock Climbing, Abseiling) reads Pull-Up Maximum and Dead Hang Duration from §E for climbing-prerequisite gating logic.
