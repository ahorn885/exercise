...
| D9 | Evidence-basis cross-check | **Name-existence check (warn-only) + mode-discriminator on evidence_basis paths (HARD ERROR — #217).** §7 schema rule: event-mode `goal_viability.evidence_basis` must reference at least one §H.2 field; no-event must reference §H.3 fields only. A mode-discriminator violation raises `Layer3BOutputError('evidence_basis_mode_violation')`; name-existence stays warn-only (parity with 3A). | Stricter than 3A's single-rule evidence_basis (3A is name-existence only). 3B's mode-discriminator on paths is spec §7 mandatory; was warn-only while the v1 §H.2 deployed-shape gap (see D11) meant the LLM had nothing real to cite. **Flipped to hard error 2026-06-22 (#217)** now that §H.2 capture shipped AND §4 hard-blocks a missing `goal_outcome` pre-LLM — so the only remaining trigger is an LLM grounding miss. Rule #15 `print()` at each raise counts the failure rate in `/admin/logs`. |
...
- Step 5: `_check_evidence_basis(payload, prep_dict, mode)` — name-existence (warn-only) + mode-discriminator on path prefixes (`h2.*` event, `h3.*` no-event). Mode-discriminator raises `Layer3BOutputError('evidence_basis_mode_violation')` per D9/#217.
...
- **Mode-discriminator check** (3B-specific): in event-mode, `goal_viability.evidence_basis` must include at least one `h2.*`-prefixed path; in no-event-mode, must NOT include any `h2.*` paths. **Violation raises `Layer3BOutputError('evidence_basis_mode_violation')` — hard, plan-failing (#217, flipped 2026-06-22).** Was warn-only telemetry under D9 until the §H.2 form-refresh landed; now that §H.2 capture shipped and §4 hard-blocks a missing `goal_outcome` pre-LLM, only an LLM grounding miss can trip it. A Rule #15 `print()` precedes each raise so `/admin/logs` can count the failure rate.
...
| L3B-P-3 | Mode-discriminator on evidence_basis paths as HARD fail (not warn) | **CLOSED 2026-06-22 (#217)** — flipped to `Layer3BOutputError('evidence_basis_mode_violation')`. Unblocked once §H.2 capture shipped + §4 hard-blocks a missing `goal_outcome` pre-LLM. Rule #15 `print()` counts the failure rate; name-existence stays warn-only. |
...
**The mode-discriminator on evidence_basis paths is now a hard, plan-failing error (#217, flipped 2026-06-22).** Resolved: the §H.2 form-refresh (L3B-P-2) landed, so the fail-hard tightening is unblocked. A no-event-mode LLM citing an `h2.*` field (or an event-mode LLM citing none) now raises `Layer3BOutputError('evidence_basis_mode_violation')`. New residual risk: a borderline-but-acceptable grounding miss now fails the plan rather than warning — the Rule #15 `print()` at each raise exists to watch the failure rate in `/admin/logs`; if it fires more than expected, revisit (e.g. single re-prompt before raising). Name-existence stays warn-only (parity with 3A).
