...
| D3 | Input format | **Full payloads verbatim** for 3A + 2A + 2D + 1 + `request` + `parsed_intent`. `prior_plan_session_window` rendered hybrid per D4. Layer 2B/2C/2E only present when `parsed_intent` triggered their re-run; full when present. | ~3500–4500 input token budget fits without trimming. T1 default cascade is narrow (3A re-run only). |
...
- **Phase decomposition.** T1 inherits the periodization shape from 3B (read via `prior_plan_session_window[*].phase_metadata` since 3B is not re-run in the T1 default cascade); no `PhaseStructure` / `SeamReview` / per-phase decomposition. The synthesizer reads "what phase are we currently in?" from the adjacent sessions' metadata.
...
### 3.5 Periodization shape — inherited from adjacent sessions

| Variable | Source | Notes |
|---|---|---|
| `inherited_phase.name` | `prior_plan_session_window` `phase_metadata` | Current phase derived from the refresh-window-after sessions' phase metadata (T1 doesn't re-run 3B; inherits shape). Required non-None — refresh-window-after must contain at least one session with `phase_metadata`. |
| `inherited_phase.intended_volume_band` | Layer 4 §6.1 + 2A | Phase-resolved volume band the inherited phase targets. The synthesizer aims T1 output volume to land inside the band when continuity is maintained. |
| `inherited_phase.intended_intensity_distribution` | Layer 4 §5.4 v1 defaults | Per-phase target distribution (Base ≈ 80/15/5; Build ≈ 70/20/10; Peak ≈ 70/20/10; Taper ≈ 75/15/10). T1 output's intensity placement reads against the phase's distribution but is not strictly enforced over a 2-day window (the validator's `intensity_dist_*` rule applies per-phase, not per-2-day-slice). |
| `inherited_phase.days_to_event` | 3B periodization (read from `prior_plan_session_window`) | Optional. When present + ≤ 14, the synthesizer should default to Taper-shape sessions per §8.5 even though Taper flags are spec-auto. |
...
- `layer3b_payload` — T1 default cascade doesn't re-run 3B; periodization shape is inherited from adjacent-session metadata per §3.5.
...
- `event_date` / `race_format` — surfaced indirectly via `inherited_phase.days_to_event` when relevant; race-week specifics are handled by `Layer4_RaceWeekBrief` (queued, last of 5 prompt bodies).
...
Phase: {{ inherited_phase.name }}
Intended volume band: {{ inherited_phase.intended_volume_band }}
Intended intensity distribution (Z1-Z2 / Z3 / Z4-Z5): {{ inherited_phase.intended_intensity_distribution }}
{{#inherited_phase.days_to_event}}
Days to event: {{ inherited_phase.days_to_event }}
{{/inherited_phase.days_to_event}}
...
| Refresh-window-after is empty (e.g., end-of-plan; athlete is on the last 9 days and refresh starts at day 7) | Continuity constraint is reduced. Read `inherited_phase` from refresh-window-during-prior metadata instead. |
| `inherited_phase` cannot be derived (no `phase_metadata` in any prior_plan_session) | Should not happen on a refresh against an existing plan — phase metadata is written by `plan_create` per §7.5. If observed, surface as `Observation(category='opportunity', text='Phase metadata missing on prior sessions — refresh proceeded with default Base assumptions')` + prescribe Base-shape sessions. |
...
| PSS-T1-14 | Empty refresh-window-after (athlete is at end-of-plan, refresh starts day 7 of remaining 9). | Continuity reads from refresh-window-during-prior + inherited_phase metadata; sessions prescribed without forward-continuity constraint. |
| PSS-T1-15 | `inherited_phase` cannot be derived (no `phase_metadata` in prior_plan_session_window). | Per §10 edge case: surface `Observation(category='opportunity', text='Phase metadata missing — refresh used Base defaults')`; prescribe Base-shape sessions. |
...
- **`inherited_phase.intended_intensity_distribution`** — when 3B is not re-run on T1, the phase distribution is read from the §5.4 v1 defaults (Base 80/15/5 etc.). If 3B was custom-tuned for the athlete in the prior `plan_create` call, the T1 reading uses the default rather than the athlete-specific shape. Document as v1 limitation; full athlete-specific shape inheritance would require persisting the per-call `intended_intensity_distribution` in `prior_plan_session_window` metadata. Lands as a follow-up when T1 telemetry reveals the gap matters.
