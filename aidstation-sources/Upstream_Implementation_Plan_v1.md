...
**Phase 3 total:** 2 sessions (substrate ✅ + driver ✅), ~10 substantive files + bookkeeping shipped. **Phase 3 complete for Layer 3A.** Layer 3B driver (Phase 4) queued next.

### Phase 4 — Layer 3B LLM driver (1-2 sessions)

| Step | Session scope | Files (est.) | Notes |
|---|---|---|---|
| **4.1** | **Layer 3B event-metadata helper** — `load_layer3b_event_metadata(db, user_id)` in `race_events_repo.py` returning `(mode, event_date, event_locale_id, race_format, time_to_event_weeks)` | 2-3 (helper + tests + closing handoff) | **The original "Layer 3B caller-side rewire" scope** from the D-72 forward-pointer. Can ship standalone OR fold into 4.2. Closes the data-access boundary half. |
| **4.2** | **Layer 3B LLM integration** — `llm_layer3b_goal_timeline_viability(...)` + paired `Layer3B_v1.md` prompt body | 6-8 (over ceiling — same precedent as 3.1) | Consumes the 4.1 helper internally to populate the Layer3BPayload event-metadata fields. Prompt body source decisions similar to 3A. **Triggers #2 + #8 expected.** Includes paired §8.3 doc-sweep fix (`open_ended` → `no-event`). |

**Phase 4 total:** ~1-2 sessions, ~8-11 files.

### Phase 5 — Orchestrator wiring (1-2 sessions)

| Step | Session scope | Files (est.) | Notes |
|---|---|---|---|
| **5.1** | **Layer 4 orchestrator vertical slice — race_week_brief** | 5-7 | New `layer4/orchestrator.py` with `orchestrate_race_week_brief(db, user_id)` that (a) loads RaceEventPayload via `load_target_race_event_payload`; (b) calls Layer 1 builder → Layer 2A-E builders → Layer 3A → Layer 3B; (c) threads everything to `llm_layer4_race_week_brief_cached`. First end-to-end vertical slice. Includes paired Layer4_Spec.md §4.5 source-pointer wording-tightening (D-72 follow-on nit). |
