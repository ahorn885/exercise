-- 0003_seed_terrain_gap_rules.sql
-- Epic #488 — bring layer0.terrain_gap_rules into the DB-native authoring model
-- (the migrations/gate loop), de-orphaning the LAST spec-sourced Layer 0 table
-- from the ad-hoc etl/sources/ one-shot SQL. With 0001 + 0002 + 0003 through the
-- gate cleanly, the §6.4 extractor/workbook freeze gate (2-3 migrations) clears.
--
-- terrain_gap_rules is spec-sourced (Layer2B_Spec.md §7), NOT ETL-emitted, so it
-- never appeared in the genesis snapshot and the layer0-gate never saw it. Unlike
-- supplement_vocabulary (0002, cone-external), terrain_gap_rules IS a 0C cone
-- table (read by Layer 2B) and already lives in _LAYER0_TABLE_FAMILY as "0C", so
-- its edits already invalidate plan-gen caches. Like every spec-sourced table it
-- stays OUT of schema.sql (the self-contained migration creates it) — here for
-- consistency with the populate-script precedent + to keep the orchestrator
-- TestLayer0TableFamilyMap.test_includes_out_of_schema_serving_tables guard true.
--
-- Subsumes the legacy one-shots (both now historical) and reproduces their exact
-- live 0C-v2.0-r2 end-state in one transaction:
--   * etl/sources/populate_terrain_gap_rules.sql            (CREATE + 16 gap rows)
--   * etl/sources/migrate_terrain_gap_rules_severity.sql    (partial -> 4-band enum)
-- The severity reclassification also runs at app boot via init_db._PG_MIGRATIONS;
-- this migration carries the same banding so genesis+migrations reproduces prod.
--
-- Idempotent: CREATE ... IF NOT EXISTS; the DELETE-by-version prefix rebuilds the
-- 0C-v2.0-r2 active set from this file on every run (D-74, mirrors
-- etl/layer0/db.py:insert_versioned); ON CONFLICT DO NOTHING + the partial-only
-- severity UPDATE no-op after the first run. Safe to re-run.
--
-- NOTE (Andy's hands): terrain_gap_rules already exists on Neon from the ad-hoc
-- populate/severity apply, so re-applying this migration is an idempotent
-- re-derivation (same end-state), not a required deploy — it exists so the
-- gate-covered history reproduces prod.
