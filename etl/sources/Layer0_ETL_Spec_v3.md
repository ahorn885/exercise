...
One row per cross-sport comparative property. As of v10, one substantive entry (`LIT_RATIO_001`); the sheet also carries 4–5 commentary rows ("EXTENSION NOTES:", schema guidance, future-property candidates) that are NOT data and must be skipped.
...
CREATE TABLE layer0.cross_sport_properties (
  ...
  source_text         TEXT,                            -- col 7 in v10 sheet
  confidence          TEXT,                            -- col 8 in v10 sheet (e.g. "Medium")
  notes               TEXT,                            -- col 9 in v10 sheet
...
**ETL parsing rule:** ingest rows where `Property ID` matches the regex `^[A-Z]+_[A-Z]+_\d{3}$` (e.g., `LIT_RATIO_001`). All non-conforming rows — empty cells, banner text like `"EXTENSION NOTES:"`, narrative commentary in C1 — are skipped silently without warning. This rule covers the current v10 commentary (R4–R7) and any future commentary added below new property rows.

**Future properties** noted in commentary as candidates (Open Item #7): `ECCENTRIC_LOAD`, `STRENGTH_PRIMACY`, `TECHNICAL_TERRAIN_DIFFICULTY`, `NAVIGATION_DEMAND`. Schema accommodates additions without re-spec; populate when scope confirms.
...
| 7 | Cross-Sport Properties extension | Sheet 8 still has 1 substantive row (LIT_RATIO_001) in v10. Candidate properties noted in commentary: ECCENTRIC_LOAD, STRENGTH_PRIMACY, TECHNICAL_TERRAIN_DIFFICULTY, NAVIGATION_DEMAND. v3 spec §4.8 ETL parser now explicitly handles commentary skip. | Other chat | Carryover from v2; parser tightened in v3 |
...
| 21 | Sheet 7 deprecation (`Athlete Profile Data Points`) | Sheet 7 has drifted significantly from `Athlete_Onboarding_Data_Spec_v2.md` v2.5 (missing Health Conditions merged structure, v2.2–v2.5 patches, Sections K/L, Groups 2/3). Banner applied to v10 marking sheet superseded. Full deletion pending field-by-field migration audit (~30 min) to confirm no orphan content. Sheet remains correctly excluded from Layer 0 ETL throughout. | Spec workstream | READY — banner applied; deletion deferred to spec hygiene pass |
