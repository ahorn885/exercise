-- migrate_supplement_vocabulary.sql
-- FC-1 / D-26: Layer 0 supplement_vocabulary table + 25 seed entries.
--
-- Source spec: Supplement_Vocabulary_Spec.md (Claude project knowledge).
-- Consumer: Layer 2E (nutrition baseline) reads athlete supplement records
-- via FK to supplement_id; cross-references contraindications[] against §B
-- health conditions and allergies for coaching flags.
--
-- ETL version tag: supp_vocab.v1.FC1
--
-- Idempotent: CREATE ... IF NOT EXISTS + ON CONFLICT (supplement_id) DO NOTHING.
-- Safe to run multiple times. No-op on second run.
...
DECLARE
  ...
  active_count INT;
BEGIN
  -- Confirm all 25 seed IDs present with this etl_version
  SELECT ARRAY_AGG(s)
    INTO missing_ids
    ...
    WHERE s NOT IN (
      SELECT supplement_id
        ...
        WHERE etl_version = 'supp_vocab.v1.FC1' AND superseded_at IS NULL
    );

  IF missing_ids IS NOT NULL AND ARRAY_LENGTH(missing_ids, 1) > 0 THEN
    RAISE EXCEPTION 'supp_vocab.v1.FC1 verify failed — missing seed IDs: %', missing_ids;
  END IF;

  -- Sanity: active count should be exactly 25
  SELECT COUNT(*)
    INTO active_count
    FROM layer0.supplement_vocabulary
    WHERE etl_version = 'supp_vocab.v1.FC1' AND superseded_at IS NULL;
  ...
  IF active_count <> 25 THEN
    RAISE EXCEPTION 'supp_vocab.v1.FC1 verify failed — expected 25 active rows, got %', active_count;
  END IF;

  RAISE NOTICE 'supp_vocab.v1.FC1 verify OK — 25 seed entries active in layer0.supplement_vocabulary';
END $$;

COMMIT;

-- ── Post-deploy sanity (optional, run separately to inspect) ────────────────
-- SELECT category, COUNT(*) FROM layer0.supplement_vocabulary
--   WHERE superseded_at IS NULL GROUP BY category ORDER BY category;
-- Expected distribution:
--   Performance: 5 | Recovery: 9 | Health: 5 | Race-day: 3 | GI: 1 | Sleep: 1 | Hormonal-Stress: 1
