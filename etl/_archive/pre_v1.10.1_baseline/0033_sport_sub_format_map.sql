...
DELETE FROM layer0.sport_sub_format_map WHERE etl_version = '0A-v1.10.1';
...
VALUES
    ...
    ( 1, 'Triathlon', 'Triathlon (Sprint)',                          false, 'Sprint',                  '0A-v1.10.1', now()),
    ( 2, 'Triathlon', 'Triathlon (Standard / Olympic)',              true,  'Standard / Olympic',      '0A-v1.10.1', now()),
    ( 3, 'Triathlon', 'Triathlon (Half / 70.3)',                     false, 'Half / 70.3',             '0A-v1.10.1', now()),
    ( 4, 'Triathlon', 'Triathlon (Full / Ironman 140.6)',           false, 'Full / Ironman 140.6',    '0A-v1.10.1', now()),
    -- Skimo (default: Individual / Team — the standard mass-start format)
    ( 5, 'Skimo', 'Skimo (Sprint)',                                  false, 'Sprint',                  '0A-v1.10.1', now()),
    ( 6, 'Skimo', 'Skimo (Vertical / VK)',                           false, 'Vertical / VK',           '0A-v1.10.1', now()),
    ( 7, 'Skimo', 'Skimo (Individual / Team)',                       true,  'Individual / Team',       '0A-v1.10.1', now()),
    ( 8, 'Skimo', 'Skimo (Long Distance / Grand Traverse)',          false, 'Long Distance / Grand Traverse', '0A-v1.10.1', now()),
    -- Long Distance / Endurance Cycling (default: Road / Gran Fondo)
    ( 9, 'Long Distance / Endurance Cycling', 'Long Distance / Endurance Cycling (Road / Gran Fondo)', true,  'Road / Gran Fondo',  '0A-v1.10.1', now()),
    (10, 'Long Distance / Endurance Cycling', 'Long Distance / Endurance Cycling (Gravel)',            false, 'Gravel',             '0A-v1.10.1', now()),
    (11, 'Long Distance / Endurance Cycling', 'Long Distance / Endurance Cycling (Enduro)',            false, 'Enduro',             '0A-v1.10.1', now()),
    (12, 'Long Distance / Endurance Cycling', 'Long Distance / Endurance Cycling (Time Trial)',        false, 'Time Trial',         '0A-v1.10.1', now()),
    (13, 'Long Distance / Endurance Cycling', 'Long Distance / Endurance Cycling (XC Mountain Biking)',false, 'XC Mountain Biking', '0A-v1.10.1', now()),
    -- Canoe / Kayak Marathon (default: ICF Competition — the standardized marathon format)
    (14, 'Canoe / Kayak Marathon', 'Canoe / Kayak Marathon (ICF Competition)',  true,  'ICF Competition', '0A-v1.10.1', now()),
    (15, 'Canoe / Kayak Marathon', 'Canoe / Kayak Marathon (Ultra-Distance)',   false, 'Ultra-Distance',  '0A-v1.10.1', now()),
    -- Open Water Marathon Swimming (default: 10km / Olympic Distance — the standard marathon-swim distance)
    (16, 'Open Water Marathon Swimming', 'Open Water Marathon Swimming (10km / Olympic Distance)', true,  '10km / Olympic Distance', '0A-v1.10.1', now()),
    (17, 'Open Water Marathon Swimming', 'Open Water Marathon Swimming (25km / Ultra Distance)',   false, '25km / Ultra Distance',   '0A-v1.10.1', now());
...
BEGIN
    ...
    IF n_rows <> 17 THEN
        RAISE EXCEPTION '0033 verify: expected 17 active rows, found %', n_rows;
    END IF;
    ...
    IF n_bad_default <> 0 THEN
        RAISE EXCEPTION '0033 verify: % parent(s) lack exactly one default', n_bad_default;
    END IF;
    ...
    IF n_orphan_subfmt <> 0 THEN
        RAISE EXCEPTION '0033 verify: % sub_format_sport(s) absent from phase_load_allocation', n_orphan_subfmt;
    END IF;
    ...
    IF n_parent_mismatch <> 0 THEN
        RAISE EXCEPTION '0033 verify: parent_sport set differs from the bridge/PLA mismatch set by % entr(ies)', n_parent_mismatch;
    END IF;
END $$;
