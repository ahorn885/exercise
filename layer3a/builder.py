...
def _section_completeness_estimate(layer1_payload: Layer1Payload) -> dict[str, float]:
    """Per-section field-population ratios for the prompt + the LLM's
    `data_density.section_completeness` reference. Pure counting against the
    deployed Layer 1 typed shape; no semantic weighting."""
    def _ratio(model: Any, field_names: list[str]) -> float:
        if model is None:
            return 0.0
        populated = sum(
            1
            for f in field_names
            if getattr(model, f, None) not in (None, [], "", {})
        )
        return round(populated / len(field_names), 2) if field_names else 0.0

    return {
        "C": _ratio(
            layer1_payload.training_history,
            [
        ...
        "D": _ratio(
            layer1_payload.discipline_baselines,
            ["running", "cycling", "swimming", "paddling", "skiing", "navigation", "technical"],
        )
        if layer1_payload.discipline_baselines is not None
        else 0.0,
        "E": _ratio(
            layer1_payload.strength_benchmarks,
            [
                "front_plank_sec",
                "dead_bug_max_reps",
                "side_plank_left_sec",
                "side_plank_right_sec",
                "pushup_max_reps",
                "bodyweight_squat_max_reps",
                "single_leg_squat_left_max_reps",
                "single_leg_squat_right_max_reps",
                "pullup_max_reps",
                "dead_hang_sec",
