...
DB_PATH = os.path.join(os.path.dirname(__file__), 'instance', 'training.db')
...
def create_tables(conn):
    conn.executescript('''
        ...
        CREATE TABLE IF NOT EXISTS conditions_log (
            ...
            face_neck TEXT,
            upper_shell TEXT,
            upper_mid_layer TEXT,
            upper_base_layer TEXT,
            lower_outer TEXT,
            lower_under TEXT,
            gloves TEXT,
            arm_warmers TEXT,
        ...
        CREATE TABLE IF NOT EXISTS equipment_matrix (
            ...
            home TEXT DEFAULT '—',
            hotel_travel TEXT DEFAULT '—',
            commercial_gym TEXT DEFAULT '—',
            climbing_gym TEXT DEFAULT '—',
            outdoors TEXT DEFAULT '—',
            partner_home TEXT DEFAULT '—',
        ...
        CREATE TABLE IF NOT EXISTS recommended_purchases (
            ...
            exercises_impacted TEXT,
            priority TEXT
        );

        CREATE INDEX IF NOT EXISTS idx_training_log_date ON training_log(date);
        CREATE INDEX IF NOT EXISTS idx_training_log_exercise ON training_log(exercise);
        CREATE INDEX IF NOT EXISTS idx_cardio_log_date ON cardio_log(date);
        CREATE INDEX IF NOT EXISTS idx_body_metrics_date ON body_metrics(date);
    ''')


def seed_current_rx(conn):
    """Seed Current Rx from the 101 exercises in the inventory."""
    exercises = [
