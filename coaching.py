...
- **athlete_profile**: Stable athlete facts the user filled in on /profile — date of birth (compute age), sex, height, primary sport, target event name+date, weekly hours target, training window. Use these as defaults when the user hasn't restated them in this request. The `coach_notes` field is free text the athlete wrote for the coach (general context + how they want the coach to communicate) — read it carefully; it often captures life constraints or equipment limitations the structured fields can't express."""
