...
def evict_on_target_event_modality_hints_change(
    db,
    user_id: int,
    *,
    cache: Layer4Cache | None = None,
) -> int:
    """`race_modality_hints` change on the target row per
    BestFitModality_Spec_v2.md §G.

    Evicts the 3 entry points that consume the modality resolver post-BM-3:
    plan_create + single_session_synthesize + race_week_brief.
    `plan_refresh` is intentionally NOT evicted — plan_refresh tier
    renderers consume a bundled Layer 2 payload that does not include the
    Layer 2 modality payload (BM-3 G1=A3 deferral). Returns the total
    count of evicted rows across all 3 entry points.

    Field-level eviction (3 invalidate_entry_point calls) rather than a
    layer-level call because no existing `_EVICTION_POLICY` tuple matches
    exactly {plan_create, single_session_synthesize, race_week_brief}.
    Mirrors `evict_on_target_event_brief_field_change`'s shape but with
    ...
    for entry_point in (
        ...
        total += cache.invalidate_entry_point(
            entry_point,
            layer='race_events_modality_hints',
            user_id=user_id,
        )
    return total
