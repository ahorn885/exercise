...
class TestSkillsRoute:
    ...
    def test_post_with_empty_vocab_no_ops_and_redirects(self, monkeypatch):
        # Defensive path: populate script not yet applied → vocab empty.
        # The save handler should still advance the athlete forward.
        app = _make_onboarding_app()
        conn = _FakeConn()
        ...
        # Vocab SELECT only; no upserts, no commit, no eviction.
        assert len(conn.calls) == 1
        assert conn.commits == 0
        assert evictions == []
        # Still redirects forward.
        assert response.status_code == 302
        assert '/onboarding/schedule' in response.location
