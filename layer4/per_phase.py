...
# The ratified `# Cardio drills` SYSTEM_PROMPT section (#698 Track 2). Defined as
# a shared constant so the plan-refresh path (`plan_refresh._synthesize_refresh_tier`)
# renders the identical instruction without duplicating the wording. Spliced into
# SYSTEM_PROMPT below (byte-identical to the prior inline copy).
CARDIO_DRILLS_PROMPT_SECTION = """# Cardio drills
...
def _clamp_sessions_per_day(
    ...
    lowest-index strengths are kept and the downstream `_repair_strength_collisions`
    resolves the surviving pair.

    Runs BEFORE `_repair_strength_collisions`/`_repair_day_composition`, which
    assume ≤2 training/day; any residual two-hard / strength+strength collision on
    the kept pair is healed there. Surviving sessions on a trimmed day are
...
def synthesize_phase(
    *,
    ...
    for pass_offset in range(remaining_budget + 1):
        ...
        # prod plan #47 class). Runs FIRST — the collision/composition repairs
        # below assume ≤2 training/day and skip any day still holding 3+.
